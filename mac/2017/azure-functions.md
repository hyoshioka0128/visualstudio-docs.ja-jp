...
1. プロジェクト名を右クリックし、 **[発行]、[Azure に発行する]** の順に選択します。![選択された AF-httptrigger プロジェクト、およびコンテキスト メニューの [発行] と [Azure に発行する] コマンドが強調表示されている Azure Functions のスクリーンショット。](media/azure-functions-image5.png)
...
4. **[新しい App Service を作成する]** ダイアログに設定を入力します。![Azure Functions の [新しい App Service] ウィンドウのスクリーンショット。Azure で新しい App Service を作成するための設定が示されています。](media/azure-functions-image7.png)
...
5. **[次へ]** をクリックし、ストレージ アカウントを作成します。 Functions の共通言語ランタイムでは、Azure ストレージ アカウントが必要です。 **[カスタム]** をクリックし、汎用のストレージ アカウントを作成するか、既存のものを使用します。
    ...
    ![Azure Functions の [ストレージ アカウントの構成] 画面のスクリーンショット。 [ストレージ アカウント] に [カスタム] が選択され、[アカウント名] と [アカウントの種類] が入力されています。](media/azure-functions-image8.png)
...
7. 発行時に "Azure で関数のバージョンを更新する" ことを求めるダイアログが表示される場合があります。 **[はい]** をクリックします。
    ...
    ![[Azure で関数のバージョンを更新する] プロンプトのスクリーンショット。"ローカル関数のバージョンと一致するように Azure アプリケーションの設定を更新する" よう求められます。](media/azure-functions-image12.png)
