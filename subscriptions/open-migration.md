...
1. VLSC にサインインして [関係サマリー] に移動します。
1. 目的のオープン ライセンスを選択します。
1. 次に、[Visual Studio サブスクリプション] の下の [ライセンスの詳細] で **[ここをクリック]** のリンクからサブスクリプションを管理するオプションを選択します。
1. 主要連絡先と同じメール アドレスを使用して、管理ポータルにサインインします。
1. これで、パートナーがオープン ライセンスのサブスクリプションを管理できるようになります。
...
1. 契約ドロップダウンの横の [+ Add agreement]\(+ 契約の追加\) を選択し、下端の **[New open license]\(新しいオープン ライセンス\)** オプションまでスクロールします。
1. ポップアップ パネルで、契約にある認証番号、ライセンス番号、お客様のメール アドレスを入力し、チェックボックスをオンにして、 **[送信]** をクリックします。
