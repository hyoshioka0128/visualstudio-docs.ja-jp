...
8. コードのさまざまな部分にブレークポイントを設定し、アプリを再開し、スタートアップ シーケンスを続行することもできます。 たとえば、*runserver.py* と *HelloFlask\_* init *_.py* の最初の行と *views.py* の行 `return "Hello Flask!"` にブレークポイントを設定します。 その後、アプリを再開し ( **[デバッグ]**  >  **[再開]** 、**Ctrl** + **Shift** + **F5** キー、または下のツール バーのボタン)、コードをステップ実行する (**F10** キー) か、**F5** キーを使って各ブレークポイントから実行します。
