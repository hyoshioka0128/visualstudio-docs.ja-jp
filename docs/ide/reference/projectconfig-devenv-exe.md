...
`/ProjectConfig` スイッチは、`/Project`、/`/Build`、`Clean`、または `/Deploy` コマンドの一部として `/Rebuild` スイッチと共に使用する必要があります。
...
次のコマンドでは、`CSharpWinApp` 内の `Debug` プロジェクト ビルド構成を使用して、プロジェクト `MySolution` をビルドします。
