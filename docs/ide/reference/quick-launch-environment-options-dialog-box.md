...
description: '[環境] セクションの [クイック起動] ページを使用して、オプション、テンプレート、メニューなどの IDE アセットのアクションをすばやく検索して実行する方法について説明します。'
...
|--------------|--------------| - |
|直前に使用|@mru<br /><br /> たとえば、`@mru font` のように指定します。|**直前に使用** した項目が最大 5 つ表示されます。|
|メニュー|@menu<br /><br /> たとえば、`@menu project` のように指定します。|検索をメニュー項目に制限します。|
|オプション|@opt<br /><br /> たとえば、`@opt font` のように指定します。|検索を **[オプション]** ダイアログ ボックスの設定に制限します。|
|Documents|@doc<br /><br /> たとえば、`@doc program.cs` のように指定します。|検索を検索条件の開いているドキュメントのファイル名とパスに限定しますが、ファイル内のテキストは検索されません。|
