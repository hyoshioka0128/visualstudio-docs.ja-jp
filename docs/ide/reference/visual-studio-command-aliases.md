...
コマンド エイリアスを利用すると、コマンドを実行するとき、入力する文字が少なくなります。 **[検索/コマンド]** ボックスまたは **[コマンド]** ウィンドウにエイリアスを入力します。 たとえば、`>File.OpenFile`[ファイルを開く] **ダイアログ ボックスを表示するときに、「** 」ではなく、定義済みのエイリアス「`>of`」を入力できます。

`alias`[コマンド] **ウィンドウで「** 」と入力すると、現在のエイリアスとその定義の一覧が表示されます。 `>cls`[コマンド] **ウィンドウの内容を消去するには、「** 」と入力します。 特定のコマンドのエイリアスを表示するには、「`alias <command name>`」と入力します。
