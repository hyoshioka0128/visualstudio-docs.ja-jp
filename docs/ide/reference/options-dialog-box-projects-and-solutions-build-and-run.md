---
title: '[ビルド/実行] ([オプション] ダイアログ - [プロジェクトおよびソリューション])'
description: Visual Studio で、同時にビルドできる C++ または C# のプロジェクトの最大数、特定の既定のビルドの動作、いくつかのビルドのログ設定を指定します。
...
ビルド ログ ファイルに書き込まれる情報量を決定します。これは、 *\\\<ProjectName>\Debug\\\<ProjectName>.log* にあります。
