...
コードが完璧に、予想通りに実行されれば、成功です。 ただし、応答しない、またはクラッシュする場合、あるいは予期しない結果になる場合、問題の原因を見つけ、バグを修正する必要があります。
...
![Visual Studio のコード ウィンドウのスクリーンショット。 左側の余白の赤い点は、黄色でマークされているコード行でのブレークポイントを示します。](../ide/media/vs_ide_gs_debug_hit_breakpoint.png)
...
![Visual Studio のコード ウィンドウのスクリーンショット。 黄色でマークされているコード行で、変数が選択されており、ドロップダウンにはその現在の値と参照が表示されます。](../ide/media/vs_ide_gs_debug_inspect_value.png)
