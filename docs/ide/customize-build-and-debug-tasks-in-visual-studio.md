---
title: JSON ファイルを使用してビルド デバッグ タスクをカスタマイズする
description: タスクをカスタマイズして、Visual Studio で認識されないコードベースを実行およびデバッグするための構成の詳細を指定する方法について説明します。
