...
description: クラス ダイアグラムを使用して、プロジェクト内の C++ コード要素、クラス、その他の型を設計および視覚化する方法について説明します。
...
型の解決に関する問題でよく表示されるエラーは、"**クラス ダイアグラム '\<element>' の 1 つ以上のシェイプに対して、コードが見つかりませんでした**" です。 このエラー メッセージは、必ずしもコードがエラーであることを意味しません。 クラス デザイナーで、コードを表示できないことだけを示しています。 次の方法を試してください。
