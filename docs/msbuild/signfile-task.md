...
description: MSBuild で SignFile タスクを使用して、指定された証明書を使用して指定されたファイルに署名する方法について説明します。
...
|`SigningTarget`|必須の <xref:Microsoft.Build.Framework.ITaskItem> 型のパラメーターです。<br /><br /> 種類が .exe または .dll の、証明書で署名するファイルを指定します。|
