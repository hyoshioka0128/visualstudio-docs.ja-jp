...
|-help|/? または -h|使用方法に関する情報を表示します。 たとえば、次のようなコマンドになります。<br /><br /> `msbuild.exe -?`|
...
## <a name="example"></a>例
 ...
 `rebuild`MyProject.proj*プロジェクトの* ターゲットをビルドする例を次に示します。
 ...
 *MSBuild.exe* を使用して、より複雑なビルドを実行できます。 たとえばそれを使用して、ソリューション内の特定のプロジェクトの特定のターゲットをビルドできます。 `NotInSolutionFolder` プロジェクトをリビルドし、`InSolutionFolder`NewFolder*ソリューション フォルダー内にある* プロジェクトを消去する例を次に示します。
