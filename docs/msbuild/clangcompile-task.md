...
description: C++ コンパイラ ツールである clang.exe をラップする、MSBuild ClangCompile タスクの目的とパラメーターについて説明します。
...
|**最適化**|省略可能な **string** 型のパラメーターです。<br/><br/>アプリケーションの最適化レベルを指定します。<br/><br/>**Custom** : カスタムの最適化。<br/>**Disabled** : 最適化を無効にします (`O0` を使います)。<br/>**MinSize** : サイズを最適化します (`Os` を使います)。<br/>**MaxSpeed** : 速度を最適化します (`O2` を使います)。<br/>**Full** : 高価な最適化 (`O3` を使います)。|
