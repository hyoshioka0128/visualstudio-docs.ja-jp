...
description: アプリケーションからのプログラムによるビルドが、Visual Studio または MSBuild.exe 内で実行されたビルドと一致することを確認する方法について説明します。
...
> [!IMPORTANT]
> MSBuildLocator を呼び出すメソッドでは、(`Microsoft.Build` 名前空間の) MSBuild 型を参照することはできません。 たとえば、以下を行うことはできません。
>
> ```csharp
> void ThisWillFail()
> {
