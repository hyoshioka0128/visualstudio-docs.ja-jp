...
![Visual Studio デバッガーのスクリーンショット。コード行が強調表示されています。 コンテキスト メニューで [関数にステップ イン] が選択され、Path.set メソッドが選択されています。](../debugger/media/dbg-tour-step-into-specific.png)

この例では、 **[関数にステップ イン]** を選択し、`Path.set` のコードに移動します。

![Visual Studio デバッガーのスクリーンショット。Path.set のコードが表示されています。 set 関数を囲む中かっこが黄色で強調表示されています。](../debugger/media/dbg-tour-step-into-specific-2.png)

## <a name="run-to-a-point-in-your-code-quickly-using-the-mouse"></a>マウスを使用し、コード内のポイントにすばやく移動する

デバッガーで、 **[クリックで実行]** (ここまで実行します) ボタン ![Visual Studio デバッガーの [クリックで実行] ボタンのスクリーンショット。 このボタンは、ボタンが配置されている行まで実行する必要があることを示しています。](../debugger/media/dbg-tour-run-to-click.png) が左側に表示されるまで、コード行をマウスでポイントします。

![Visual Studio デバッガーのスクリーンショット。[クリックで実行] ボタンが Update 関数の呼び出しのすぐ左側に表示されています。](../debugger/media/dbg-tour-run-to-click-2.png)
