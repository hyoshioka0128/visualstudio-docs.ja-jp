...
### <a name="remote-debugging-security"></a>リモート デバッグのセキュリティ
 ...
 Windows 認証モードを使用する場合は、msvsmon をホストしているコンピュータ上のすべてのアクセス許可がユーザーに付与されるため、信頼されていないユーザーに msvsmon への接続許可を与えることは危険です。

 リモート マシン上の不明なプロセスをデバッグしない: デバッガーを実行しているコンピューターに影響を与える可能性のある悪用、または msvsmon を侵害する可能性があります。 それでも未知のプロセスをデバッグする必要がある場合は、ローカルでデバッグし、セキュリティ上の脅威をローカルの問題に抑えるためにファイアウォールを使用します。

 msvsmon の構成については、リモート[デバッガーのセットアップを](../debugger/remote-debugging.md#bkmk_setup)参照してください。
...
- ソース サーバー。ソース コードのレポジトリからソース コードのバージョンを表示します。 最新バージョンのプログラム ソース コードを持っていないときに便利です。 [セキュリティ警告: デバッガは信頼できないコマンドを実行する必要があります](../debugger/security-warning-debugger-must-execute-untrusted-command.md)。
