...
2. **アプリの & 機能**の設定の右側にある [**オプション機能**] ([**アプリ & 機能**] の下) を選択します。

   **オプションの機能**の設定が表示されます。

3. [**オプション機能**] 設定で、[**機能の追加**] を選択します。 インストールできるオプション機能の一覧が表示されます。
...
## <a name="using-graphics-diagnostics-for-the-first-time"></a>グラフィックス診断を初めて使用する
 これで、必要なものがすべて揃い、グラフィックス診断の使用を開始する準備が整いました。 必要な作業は、これらの手順に従うだけです。
