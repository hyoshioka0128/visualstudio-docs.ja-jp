---
title: コマンド ラインからシンボル ファイルの場所を指定する
description: 関数名や行番号などのシンボル情報を表示するには、VSPerfReport コマンドライン ツールでシンボル (.pdb) ファイルにアクセスできなければなりません。そのしくみについて学習します。
