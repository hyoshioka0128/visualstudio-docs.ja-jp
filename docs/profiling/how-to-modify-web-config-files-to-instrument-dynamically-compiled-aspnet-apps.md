---
title: Web.Config ファイル - 動的 ASP.NET アプリのインストルメント化とプロファイリング
description: Visual Studio プロファイル ツールを使用して、動的にコンパイルされた ASP.NET Web アプリケーションのタイミングとメモリ アクティビティ データを収集する方法について説明します。
