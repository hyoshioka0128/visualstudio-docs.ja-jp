---
title: インストルメント化前のコマンドおよびインストルメント化後のコマンドを指定する | Microsoft Docs
description: パフォーマンス セッションのバイナリがインストルメント化される前か後に実行されるコマンドを指定する方法について学習します。
ms.date: 11/04/2016
ms.topic: how-to
f1_keywords:
