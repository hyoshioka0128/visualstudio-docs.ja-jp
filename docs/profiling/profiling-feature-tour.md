...
デバッグ中にメモリ使用量を分析するには、少なくとも 1 枚のメモリ スナップショットを取得する必要があります。 多くの場合、メモリを分析する最良の方法は 2 枚のスナップショットを取得することです。疑われるメモリ問題の発生直前に 1 枚、発生直後に 1 枚取得します。 それから、2 枚のスナップショットの違いを表示し、厳密に何が変化しているのか確認します。
...
![パフォーマンス エクスプローラー ツール](../profiling/media/prof-tour-performance-explorer.png "パフォーマンス エクスプ ローラー")
...
|----------------------|---------------------|-------------|-------------|
|[CPU 使用率](../profiling/cpu-usage.md)|はい|はい|はい|
|[メモリ使用量](../profiling/memory-usage.md)|はい|はい|はい|
|[.NET オブジェクトの割り当て](../profiling/dotnet-alloc-tool.md)|はい (.NET のみ)|はい|はい|
|[GPU 使用率](/visualstudio/debugger/graphics/gpu-usage)|はい|はい|いいえ|
|[アプリケーションのタイムライン](../profiling/application-timeline.md)|はい|はい|いいえ|
|[パフォーマンスのヒント](../profiling/perftips.md)|はい|XAML の場合は可、HTML の場合は不可|はい|
|[パフォーマンス エクスプ ローラー](../profiling/performance-explorer.md)|はい|いいえ|はい|
...
|[パフォーマンス エクスプ ローラー](../profiling/performance-explorer.md)|はい|いいえ|はい|
|[[IntelliTrace]](../debugger/intellitrace.md)|Visual Studio Enterprise を使用した .NET のみ|Visual Studio Enterprise を使用した .NET のみ|Visual Studio Enterprise を使用した .NET のみ|
|[ネットワーク使用率](../profiling/network-usage.md)|いいえ|はい|いいえ|
|[HTML UI の応答性](../profiling/html-ui-responsiveness.md)|いいえ|HTML の場合は可、XAML の場合は不可|いいえ|
|[JavaScript メモリ](../profiling/javascript-memory.md)|いいえ|HTML の場合は可、XAML の場合は不可|いいえ|
::: moniker-end
