...
|[DA0012: リフレクションが頻繁に実行されています](../profiling/da0012-significant-amount-of-reflection.md)|<xref:System.Reflection?displayProperty=fullName> や <xref:System.Reflection.IReflect.InvokeMember%2A> など、<xref:System.Reflection.IReflect.GetMember%2A> メソッドの呼び出しか、<xref:System.Type.InvokeMember%2A> など、Type メソッドの呼び出しがプロファイリング データの大きな割合を占めています。 可能な場合は、事前バインディングを使用するこれらのメソッドを、依存アセンブリのメソッドに置き換えることを検討してください。|
