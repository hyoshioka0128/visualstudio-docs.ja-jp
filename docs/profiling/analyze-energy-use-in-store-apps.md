...
- **C#/VB/C++ と XAML** の「[接続状態とコストの管理](/previous-versions/windows/apps/hh452985\(v\=win.10\))」セクションでは、ネットワーク接続情報を提供する Windows API について説明されています。この情報を使って、アプリはネットワーク トラフィックのコストを最小限にすることができます。
