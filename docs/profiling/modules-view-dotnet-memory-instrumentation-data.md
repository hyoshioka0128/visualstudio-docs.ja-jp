...
description: インストルメンテーション メソッドを使用して収集された .NET メモリの割り当てデータ (メモリとタイミング データ) が、[モジュール] ビューでどのようにモジュールごとにグループ化されるかについて学習します。
ms.date: 11/04/2016
ms.topic: conceptual
helpviewer_keywords:
- Modules view
...
|**アプリケーション時間 % (子を含む)**|プロファイリング実行の包括経過時間の総計に対する、このモジュールまたは関数のアプリケーション包括時間の割合。|
