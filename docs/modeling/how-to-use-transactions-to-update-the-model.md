...
description: トランザクションによって、ストアに加えられた変更がグループとして扱われること、およびトランザクションを使用してモデルを更新する方法が確認されます。
...
トランザクションでは、ストアに加えられた変更がグループとして扱われるようにします。 グループ化された変更は、1つの単位としてコミットまたはロールバックできます。
 ...
 プログラムコードによって、Visual Studio の視覚化およびモデリング SDK でストアの要素が変更、追加、または削除されるたびに、トランザクション内で実行する必要があります。 <xref:Microsoft.VisualStudio.Modeling.Transaction>変更が発生したときに、ストアに関連付けられているのアクティブなインスタンスが存在している必要があります。 これは、すべてのモデル要素、リレーションシップ、図形、図、およびそれらのプロパティに適用されます。
