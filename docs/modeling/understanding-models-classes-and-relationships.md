...
description: Dsl 定義ファイルによってドメイン固有言語 (DSL) がどのように定義されているか、および DSL ソリューションのプログラムコードのほとんどがこのファイルから生成される方法について説明します。
...
## <a name="multiplicities"></a>多
 多重度は、ドメインリレーションシップで同じロールを持つことができる要素の数を指定します。 この例では、カタログロールの0対多 (0. \* .) の多重度設定によって、**発行元** ドメインクラスの任意のインスタンスが、指定した数の **PublisherCatalog** リレーションシップリンクを持つことができることが指定されています。
