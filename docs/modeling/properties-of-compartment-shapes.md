...
description: コンパートメント図形は、ドメイン固有言語でドメインクラスを表示するために使用できる図形の1つであることについて説明します。
...
|プロパティ|説明|Default|
|-|-|-|
|既定の展開/折りたたみ状態|の場合 `Expanded` 、コンパートメントは作成時に表示されます。 `Collapsed`の場合、これらは無効です。|展開|
...
|単一のコンパートメントヘッダーが表示されています|の場合、図形に1つのコンパートメントがある場合 `False` 、コンパートメントのヘッダーは表示されません。|True|
...
|アクセス修飾子|コンパートメントシェイプ (または) のアクセスレベル `public` `internal` 。|パブリック|
...
|継承修飾子|コンパートメントシェイプ ( `none` 、または) から生成されるソースコードクラスの継承の種類について説明し `abstract` `sealed` ます。|なし|
