...
description: Visual Studio でドメイン固有言語 (DSL) のインスタンスまたはモデルを保存するときに作成または更新された XML ファイルについて説明します。
...
> [!NOTE]
> 特定の形式でモデルを保存するが、そのフォームから再読み込みを行う必要がない場合は、カスタムのシリアル化スキームではなく、テキストテンプレートを使用してモデルからの出力を生成することを検討してください。 詳細については、「 [Domain-Specific 言語からのコードの生成](../modeling/generating-code-from-a-domain-specific-language.md)」を参照してください。
...
-   =  属性値としてではなく、要素として保存されたドメインプロパティを持つように表現 **要素** を設定します。
