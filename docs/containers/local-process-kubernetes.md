---
title: Visual Studio を使用して Local Process with Kubernetes を利用する
titleSuffix: ''
...
* [Local Process for Kubernetes 拡張機能がインストールされていること][lpk-extension]。
...
Visual Studio で [自転車共有サンプル アプリケーション][bike-sharing-github]から *mindaro/samples/BikeSharingApp/ReservationEngine/app.csproj* を開きます。

次の図のように、プロジェクトで起動設定のドロップダウンから *[Local Process with Kubernetes]* を選択します。

![Local Process with Kubernetes を選択する](media/local-process-kubernetes/choose-local-process.png)
...
![Local Process with Kubernetes クラスターを選択する](media/local-process-kubernetes/choose-local-process-cluster.png)
...
![接続された開発用コンピューター](media/local-process-kubernetes/development-computer-connected.png)
...
診断ログは、開発用コンピューターの *TEMP* ディレクトリの `Local Process with Kubernetes` ディレクトリにあります。 
...
Local Process with Kubernetes のしくみを理解する

> [!div class="nextstepaction"]
> [Local Process with Kubernetes のしくみ](overview-local-process-kubernetes.md)
