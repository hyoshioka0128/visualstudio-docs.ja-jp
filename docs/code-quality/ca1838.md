---
title: 'CA1838: P/Invoke の StringBuilder パラメーターを使用しないでください'
ms.date: 08/03/2020
ms.topic: reference
f1_keywords:
...
この規則は既定で無効になっています。違反が関心のあるかどうかをケースごとに分析し、違反に対処するための非自明なリファクタリングが必要になる可能性があるためです。 ユーザーは、 [analyzer ルールの重大度](use-roslyn-analyzers.md#rule-severity)を構成することによって、このルールを明示的に有効にすることができます。
