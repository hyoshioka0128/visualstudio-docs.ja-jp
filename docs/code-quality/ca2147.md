...
## <a name="cause"></a>原因
<xref:System.Security.SecurityTransparentAttribute> としてマークされているコードには、アサートするための十分なアクセス許可が付与されていません。
...
## <a name="example"></a>例
`SecurityTestClass` が透過的な場合、`Assert` メソッドが <xref:System.InvalidOperationException>をスローすると、このコードは失敗します。
