...
ms.locfileid: "81649295"
---
# <a name="native-recommended-rules-rule-set"></a>"ネイティブ推奨規則" 規則セット

ネイティブ推奨ルールは、潜在的なセキュリティ ホールやアプリケーションのクラッシュなど、ネイティブ コードで最も重大で一般的な問題に焦点を当てています。 このルール セットには、[ネイティブ最小](native-minimum-rules-rule-set.md)規則のルール セット内のすべてのルールが含まれます。

この規則セットは、ネイティブ プロジェクト用に作成するカスタム規則セットに含めます。
