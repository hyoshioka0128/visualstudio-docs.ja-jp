...
- 値がソフトウェアによって保存およびアクセスされる (ファイルまたはデータベースに保存される) 場合は、インバリアントカルチャを使用します。 [https://docs.microsoft.com/azure/active-directory/develop/scenario-protected-web-api-overview](<xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=nameWithType> ) をご覧ください。
...
この規則違反を修正するには、引数を受け取るオーバーロードを使用し <xref:System.IFormatProvider> ます。 または、 [C# の補間文字列](/dotnet/csharp/tutorials/string-interpolation)を使用して、メソッドに渡し <xref:System.FormattableString.Invariant%2A?displayProperty=nameWithType> ます。
