...
ms.locfileid: "90560763"
---
# <a name="configure-net-code-quality-analyzers-with-editorconfig"></a>EditorConfig を使用した .NET コード品質アナライザーの構成

.NET 5.0 以降では、 [.NET SDK](/dotnet/fundamentals/productivity/code-analysis.md/#code-quality-analysis)にコード品質アナライザー ([ `CAxxxx` ルール](code-analysis-for-managed-code-warnings.md)) が含まれています。 各 code quality analyzer は、構成可能なオプションを使用して、コードベースの部分に適用するように調整できます。 各オプションは、キーと値のペアを [Editorconfig](https://editorconfig.org) ファイルに追加することによって指定します。 構成ファイルは、ファイル、プロジェクト、ソリューション、またはリポジトリ全体に固有のものにすることができます。
...
## <a name="options-for-net-code-quality-analyzers"></a>.NET コード品質アナライザーのオプション
