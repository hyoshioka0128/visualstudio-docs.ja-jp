...
<xref:System.FlagsAttribute?displayProperty=fullName> が適用されていない列挙型では、値が0のメンバーは定義されません。 または、適用されている <xref:System.FlagsAttribute> を持つ列挙体は、値が0でも、名前が ' None ' ではないメンバーを定義します。 または、列挙体は、ゼロ値の複数のメンバーを定義します。
...
<xref:System.FlagsAttribute> 適用された列挙体に0値のメンバーが定義されている場合、列挙体に値が設定されていないことを示すには、その名前を ' None ' にする必要があります。 他の目的に0値のメンバーを使用することは、との <xref:System.FlagsAttribute> を使用することとは異なります。この場合、and および or のビットごとの演算子は、メンバーでは役に立ちません。 これは、1つのメンバーに値0を割り当てる必要があることを意味します。 フラグ属性付き列挙で値0を持つメンバーが複数ある場合、`Enum.ToString()` は0以外のメンバーに対して正しくない結果を返します。
