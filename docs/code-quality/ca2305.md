...
`LosFormatter`は安全ではないため、セキュリティで保護することはできません。 詳細については、「 [Binaryformatter セキュリティガイド](/dotnet/standard/serialization/binaryformatter-security-guide)」を参照してください。
...
`LosFormatter`は安全ではないため、セキュリティで保護することはできません。
