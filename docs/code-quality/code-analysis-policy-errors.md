...
- Visual Studio でのプロジェクトのローカル規則セットには、プロジェクト規則セットよりも制限の緩い**アクション**設定があります。たとえば、サーバーの **[アクション**=**エラー** ] に設定されている規則は、visual studio で実行されている規則セットで **[警告]** または **[なし**]**に設定さ**れています。
