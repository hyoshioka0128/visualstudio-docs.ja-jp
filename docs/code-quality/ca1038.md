...
この規則違反を修正するには、インターフェイスプロパティを明示的に実装します (`IEnumerator.Current`として宣言します)。 `Current`として宣言された、厳密に型指定されたパブリックバージョンのプロパティを追加し、厳密に型指定されたオブジェクトを返すようにします。
