...
この規則では、、、などの [破棄](/dotnet/csharp/discards) シンボルを使用してという名前のパラメーターにフラグを付けません `_` `_1` `_2` 。 これにより、シグネチャの要件に必要なパラメーター (デリゲートとして使用されるメソッド、特別な属性を持つパラメーターなど)、またはフレームワークによって実行時に暗黙的に値にアクセスされるがコードで参照されていない値を持つパラメーターの警告ノイズが減少します。
