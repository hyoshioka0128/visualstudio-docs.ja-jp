...
- この型は、シリアル化コンストラクターを実装します。つまり、<xref:System.Runtime.Serialization.SerializationInfo?displayProperty=fullName>を持つコンストラクター、<xref:System.Runtime.Serialization.StreamingContext?displayProperty=fullName> パラメーターシグネチャを実装しますが、基本型のシリアル化コンストラクターを呼び出しません。
