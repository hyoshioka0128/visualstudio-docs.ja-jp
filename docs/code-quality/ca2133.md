...
description: メソッドは、SecurityCritical 属性でマークされているデリゲートを、透過的であるか、SecuritySafeCritical 属性でマークされたメソッドにバインドするか、または透過的なデリゲートまたはセーフクリティカルなデリゲートをクリティカルメソッドにバインドします。
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
- CA2133
