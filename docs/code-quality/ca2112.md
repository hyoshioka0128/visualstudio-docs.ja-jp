...
次の例は、セキュリティで保護されていないフィールドを持つライブラリの種類 (`SecuredTypeWithFields`) と、ライブラリ型のインスタンスを作成できる型 (`Distributor`)、および型に対して、インスタンスを作成するためのアクセス許可を持たない型のインスタンスを作成することができます。
