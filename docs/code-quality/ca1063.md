...
- `Dispose()`メソッドが public、 [sealed](/dotnet/csharp/language-reference/keywords/sealed)、または named **Dispose**ではありません。
