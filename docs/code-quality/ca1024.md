...
<xref:System.Diagnostics.DebuggerBrowsableAttribute?displayProperty=fullName>を適用することによって、デバッガーが自動的にプロパティを展開しないようにすることができます。 次の例は、この属性がインスタンスプロパティに適用されていることを示しています。
