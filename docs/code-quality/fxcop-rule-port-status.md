...
以前に Visual Studio で静的コード分析を使用していた場合は[、FxCop アナライザ](install-fxcop-analyzers.md)ーとして現在の実装でどの規則が使用できるか疑問に思うかもしれません。 このページには、移植されるルールと、移植されていないルールと、移植する計画があるかどうかが表示されます。

## <a name="ported-rules"></a>移植された規則

roslyn アナライザーのレポの[自動生成ドキュメント ページ](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md)には、FxCop アナライザーに移植されたルールの最新のリストがあります。 このページには、ルールが既定で有効になっているかどうか、関連する*コード修正*が含まれているかどうかなどの追加情報もあります。 ([コード修正](../ide/quick-actions.md)プログラムは、Visual Studio の電球アイコン メニューで使用できるワンクリック修正です)。
...
[CA1014](ca1014-mark-assemblies-with-clscompliantattribute.md) | アセンブリを CLS 準拠でマークする
[CA1016](ca1016-mark-assemblies-with-assemblyversionattribute.md) | アセンブリバージョンでアセンブリをマークする
[CA1017](ca1017-mark-assemblies-with-comvisibleattribute.md) | アセンブリを可視でマークする
...
[CA1028](ca1028-enum-storage-should-be-int32.md) | 列挙型ストレージは Int32 である必要があります。
...
[CA1052](ca1052-static-holder-types-should-be-sealed.md) | 静的ホルダータイプは静的または継承不可である必要があります。
[CA1053](ca1053-static-holder-types-should-not-have-constructors.md) | 静的ホルダー型にはコンストラクターを含めないようにする (CA1053 は、FxCop アナライザー用の[CA1052](ca1052-static-holder-types-should-be-sealed.md)の一部です)
[CA1054](ca1054-uri-parameters-should-not-be-strings.md) | URI パラメータは文字列であってはならない
[CA1055](ca1055-uri-return-values-should-not-be-strings.md) | URI 戻り値は文字列にできません
[CA1056](ca1056-uri-properties-should-not-be-strings.md) | URI プロパティは文字列にできません
[CA1058](ca1058-types-should-not-extend-certain-base-types.md) | 型は、一定の基本型を拡張することはできません
[CA1060](ca1060-move-p-invokes-to-nativemethods-class.md) | pinvoke をネイティブ メソッド クラスに移動する
...
CA1066 | 型{0}は等しいをオーバーライドするため\<、IEquatable T>を実装する必要があります。
CA1067 | IEquatable T>を実装するときにオブジェクトをオーバーライド\<します。
...
[CA1309](ca1309-use-ordinal-stringcomparison.md) | 序数文字列比較を使用する
...
[CA1507](ca1507.md) | シンボル名を表すために nameof を使用する
CA1508 | デッド条件コードを回避する
CA1509 | コード メトリックス ルール仕様ファイルのエントリが無効です
...
[CA1724](ca1724-type-names-should-not-match-namespaces.md) | 型名は名前空間と一致しない
...
[CA1802](ca1802.md) | 必要に応じてリテラルを使用する
...
[CA1816](ca1816.md) | Dispose メソッドは、抑制終了を呼び出す必要があります。
[CA1819](ca1819.md) | プロパティは、配列を返すことはできません
[CA1820](ca1820.md) | 文字列の長さを使用して空の文字列をテストします
[CA1821](ca1821.md) | 空のファイナライザーを削除する
...
CA1825 | 長さ 0 の配列割り当てを避けます。
CA1826 | インデックス付け可能なコレクションでは、列挙可能なメソッドを使用しないでください。 代わりに、コレクションを直接使用します。
[CA2000](ca2000.md) | スコープを失う前にオブジェクトを破棄
[CA2002](ca2002.md) | 弱い ID を伴うオブジェクト上でロックしません
[CA2007](ca2007.md) | 待機中のタスクで ConfigureAwait を呼び出す方法を検討してください。
CA2008 | タスク スケジューラを渡さずにタスクを作成しない
CA2009 | 変更可能なコレクションの値を呼び出さない
CA2010 | PreserveSigAttribute でマークされたメソッドによって返される値を常に消費する
...
[CA2153](ca2153.md) | 破損状態例外をキャッチしない
...
[CA2219](ca2219.md) | finally 句で例外を発生しない
[CA2224](ca2224.md) | オーバーロード演算子の上書きと等しい
...
[CA2231](ca2231.md) | オーバーライドする値型のオーバーロード演算子が等しい
[CA2234](ca2234.md) | 文字列の代わりにシステム uri オブジェクトを渡す
...
[CA3076](ca3076.md) | 安全でない XSLT スクリプト処理。
[CA3077](ca3077.md) | API デザイン、Xml ドキュメント、および XmlTextReader でのセキュリティで保護されていない処理
[CA3147](ca3147.md) | 偽造防止トークンを検証して動詞ハンドラーをマークする
[CA5350](ca5350.md) | 脆弱な暗号アルゴリズムを使用しないでください
[CA5351](ca5351.md) | 壊れた暗号化アルゴリズムを使用しない
CA5358 | 安全ではない暗号モードを使用しないでください
CA5359 | 証明書の検証を無効にしない
CA5360 | 逆シリアル化で危険なメソッドを呼び出さない
CA5361 | 強い暗号の S チャネル使用を無効にしないでください。
CA5362 | シリアル化可能なクラスで自己参照しない
CA5363 | 要求の検証を無効にしない
CA5364 | 非推奨のセキュリティ プロトコルを使用しない
CA5365 | HTTP ヘッダーチェックを無効にしない
CA5366 | データセット読み取り XML に XmlReader を使用する
CA5367 | ポインター フィールドを使用して型をシリアル化しない
CA5368 | ページから派生したクラスの ViewStateUserKey を設定します
CA5369 | 逆シリアル化に XmlReader を使用する
CA5370 | 検証リーダーに XmlReader を使用する
CA5371 | スキーマの読み取りに XmlReader を使用する
CA5372 | XPath ドキュメントに Xml リーダーを使用する
CA5373 | 廃止されたキー派生関数を使用しません
CA5374 | Xsl 変換を使用しない
CA5375 | アカウント共有アクセス署名を使用しない
CA5376 | 共有アクセス プロトコルを使用する HttpsOnly
CA5377 | コンテナレベルのアクセスポリシーを使用する
CA5378 | ServicePointManagerSecurityProtocols を無効にしません
CA5379 | 弱いキー派生関数アルゴリズムを使用しない
CA9999 | アナライザのバージョンが一致しません

## <a name="unported-rules"></a>移植されていないルール

[FxCop アナライザ](install-fxcop-analyzers.md)ーに移植されていない一連のルールは、まだ[移植](#rules-that-may-be-ported)されていないルールと、非推奨で[移植されない](#deprecated-rules)ルールで構成されます。

### <a name="rules-that-may-be-ported"></a>移植可能なルール

次の FxCop レガシ分析ルールは、アナライザとしてまだ実装されていませんが、まだ実装されている可能性があります。 これは、技術的な理由がブロックされているか、単にルールの優先度が低いという理由が考えられます。 各ルールの移植ステータスの詳細については、[**問題の追跡**] 列のリンクをクリックしてください。
...
### <a name="deprecated-rules"></a>非推奨のルール

次の FxCop レガシ分析ルールは非推奨となり、アナライザーとして実装されません。 詳細については[、roslyn アナライザの GitHub 問題ページ](https://github.com/dotnet/roslyn-analyzers/issues?utf8=%E2%9C%93&q=is:issue+label:FxCop-Port)でルール ID **(CA1009**など) で検索できます。
...
- [CA2220](ca2220.md)
- [CA2221](ca2221.md)
- [CA2222](ca2222.md) ([正当化](https://github.com/dotnet/roslyn-analyzers/issues/1378))
- [CA2223](ca2223.md)
- [CA2228](ca2228.md)
- [CA2230](ca2230.md)
- [CA2233](ca2233.md)
...
- [ルール](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md)
