...
この規則違反を修正するには、メソッドに <xref:System.GC.KeepAlive%2A> への呼び出しを追加し、現在のインスタンス (およびC# C++の`this`) を引数として渡します。 オブジェクトをガベージコレクションから保護する必要があるコードの最後の行の後に、呼び出しを配置します。 <xref:System.GC.KeepAlive%2A>の呼び出しの直後に、オブジェクトは、マネージ参照がないと仮定して、ガベージコレクションの準備が整っていると見なされます。
