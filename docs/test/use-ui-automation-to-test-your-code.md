...
コーディングよりテストに集中している場合でも、特殊な**コード化されたテスト ビルダー**とエディターによって、簡単にコード化された UI テストを作成および編集できます。 ただし、開発者が高度な方法でテストを拡張する場合は、コードはコピーして調整する作業がわかりやすくなるように構成されます。 たとえば、Web サイトで 1 つの商品を購入するテストを記録した後、生成されたコードを編集することにより、複数の商品を購入するループを追加することができます。
