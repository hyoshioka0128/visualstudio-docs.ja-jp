...
description: IntelliTest はテスト データおよび単体テストのスイートを生成するために .NET コードを確認します。 IntelliTest を実行して失敗しているテストを確認し、それらを解決する方法について説明します。
...
**A:** はい。前提事項は、特定のメソッドの単体テストに、どのテスト データが必要ないかを指定するために使用します。 <xref:Microsoft.Pex.Framework.PexAssume> クラスを使用して前提事項を追加します。 たとえば、`lengths` 変数が NULL ではないという前提事項は次のように追加します。
