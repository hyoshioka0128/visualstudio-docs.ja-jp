...
description: ロード テスト エディターの [プロパティ] ウィンドウで [使用するエージェント] プロパティを設定して、シナリオで使用するエージェントを指定する方法について説明します。
...
> [!TIP]
> エージェントをリモート サイトに物理的に配置するのではなく、ネットワーク エミュレーションを使用して低速なネットワークをエミュレートする方法もあります。 詳細については、「[仮想ネットワークの種類の指定](../test/specify-virtual-network-types-in-a-load-test-scenario.md)」を参照してください。
