...
2. テスト エージェントを実行するための 2 つのオプションが提示されます。
   ...
   - **[サービス]** デスクトップと対話する自動テスト (コード化された UI テスト、テスト実行時のビデオ記録の作成など) を実行する必要がない場合は、 **[テスト エージェントの実行モード]** の **[サービス]** を選択します。 テスト エージェントがサービスとして起動されます。 **[次へ]** を選択します。
...
|-|-----------------|
|テストの実行中|テストの実行|
|Ready|テストの実行またはデータおよび診断情報を収集できる状態です。|
|オフライン|テストの実行またはデータおよび診断情報を収集できない状態です。|
|[Disconnected]\(切断済み\)|テスト エージェントは起動されていません。|
