...
---
# <a name="nuget-versus-sdk-as-a-project-reference"></a>プロジェクト参照としての NuGet 対 SDK

この記事は、開発者が、NuGet パッケージとしてソフトウェアをパッケージ化するか、ソフトウェア開発キット (SDK) としてパッケージ化するかを選択できるように設計されています。 具体的には、Visual Studio プロジェクトで参照される 2 つの違いについて説明します。

- [NuGet](/nuget)は、プロジェクトにライブラリを組み込むプロセスを簡略化するオープン ソースパッケージ管理システムです。 NET (.NET Core を含む) の場合、NuGet は、コードを共有するためのマイクロソフトでサポートされているメカニズムです。 NuGet では、.NET のパッケージの作成方法、ホストされるパッケージ、および使用方法を定義し、これらの各ロールのツールを提供します。 Visual Studio では、[パッケージ マネージャー](/nuget/consume-packages/install-use-packages-visual-studio)のユーザー インターフェイスを使用して、プロジェクトに NuGet パッケージを追加します。

- [SDK](../extensibility/creating-a-software-development-kit.md)は、Visual Studio が 1 つの参照項目として扱うファイルのコレクションです。 Visual Studio の [参照マネージャー] ダイアログ ボックスには、[**参照の追加**] を選択すると、現在のプロジェクトに関連するすべての SDK が一覧表示されます。 プロジェクトに SDK を追加すると、IntelliSense、ツールボックス ウィンドウ、デザイナー、オブジェクト ブラウザー、MSBuild、配置、デバッグ、およびパッケージを使用して、その SDK のすべてのコンテンツにアクセスできます。
...
| MSBuild は、.h ファイルまたは .lib ファイルを自動的に使用します。 | Y | *SDKName.props*ファイルは、Visual C++ ディレクトリのセットアップ方法などを *、.h*ファイルまたは *.lib*ファイルの自動使用に関する情報として指定します。 | N | |
| MSBuild は *、.js*ファイルまたは *.css*ファイルを自動的に使用します。 | Y | **ソリューション エクスプローラー**で、JavaScript SDK 参照ノードを展開して個々の *.js*ファイルまたは`<source include/>`*.css*ファイルを表示し、それらのファイルをソース ファイルにドラッグしてタグを生成できます。 SDK は、F5 キーを使用する方法と自動的なパッケージ設定方法をサポートします。 | Y | |
...
| このメカニズムでは、"not to copy" (コピーしない) を指定できます。 | Y | ファイルが*\redist*フォルダか*\designtime*フォルダのどちらにドロップされるかによって、使用するアプリケーションのパッケージにコピーするファイルを制御できます。 | N | どのファイルをコピーするかを、パッケージ マニフェスト内で宣言します。 |
...
| このメカニズムでは、ネイティブ WinMD に関する登録情報の指定がサポートされています。 | Y | .winmd ファイルと .dll ファイルの間の相関関係を*SDKManifest.xml*で指定できます。 | N | |
...
| このメカニズムは、非参照ファイルを配置します (たとえば、[!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] アプリケーションのテストを実行するためのテスト フレームワークを配置します)。 | Y | *\redist*フォルダにファイルをドロップすると、ファイルは自動的に配置されます。 | Y | |
...
| この機構には、パッケージを作成および使用するためのスタンドアロン*の .exe*ファイルが含まれています。 | Y | SDK には*MSBuild.exe が*含まれています。 | Y | |
...
| このメカニズムでは、軽量のマニフェスト ファイル形式がサポートされています。 | Y | *SDKManifest.xml は*多くの属性をサポートしていますが、通常は小さなサブセットが必要です。 | Y | |
