...
| MSBuild は、.h ファイルまたは .lib ファイルを自動的に使用します。 | Y | SDKName ファイルは、 ディレクトリを Visual C++ 設定する方法を Visual Studio に指示します。これは、自動 *.h* ファイルまたは *.lib* ファイルの使用についても同様です。 | N | |
