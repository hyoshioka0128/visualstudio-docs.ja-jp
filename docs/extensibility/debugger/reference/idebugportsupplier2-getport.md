---
title: IDebugポートサプライヤー2::ゲットポート |マイクロソフトドキュメント
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
## <a name="return-value"></a>戻り値
 成功した場合は`S_OK`、 を返します。それ以外の場合は、エラー コードを返します。 指定`E_PORTSUPPLIER_NO_PORT`された識別子を持つポートが存在しない場合に返します。
