...
LSP は時間の経過とともに進化し、現在ではバージョン 3.0 です。 これは、C# に豊富な編集機能を提供するために、言語サーバーの概念が OmniSharp によって取り上げられたときに始まりました。 最初に、オムニシャープは JSON ペイロードを持つ HTTP プロトコルを使用し[、Visual Studio コード](https://code.visualstudio.com)を含むいくつかのエディターに統合されています。
