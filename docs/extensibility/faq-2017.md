...
Visual Studio 2017 を使用すると、インストールされているワークロードとコンポーネントをユーザーが制御できるようになり、Visual Studio のより高速かつ軽い影響をインストールできます。 拡張機能に必要なコンポーネントがインストールされていることを確認するには、VSIX v3 と Visual Studio 2017 を使用して、拡張機能が依存するコンポーネントを前提条件として宣言する必要があります。 必要な前提条件のいずれかがユーザーのコンピューターで利用できない場合、ユーザーは拡張機能を実行できません。 ただし、不足しているコンポーネントを必要とする拡張機能をユーザーがインストールしようとしていることが Visual Studio によって検出された場合は、必要なコンポーネントをインストールするように求めるメッセージが表示されます。 オプションのコンポーネントがある場合、実行時に適切な機能の検出を実行すると、必須コンポーネントを前提条件として一覧表示する必要がありません。 すべての拡張機能は、コアエディターコンポーネントを依存関係、`Microsoft.VisualStudio.Component.CoreEditor`として指定する必要があります。
...
コンポーネント Id の一覧については、「 [Visual Studio 2017 のワークロードとコンポーネント id](/visualstudio/install/workload-and-component-ids?view=vs-2019)」を参照してください。 どのコンポーネントには、特定のバイナリが含まれていますがわからない場合は、ダウンロード、[コンポーネント バイナリ マッピング スプレッドシート](https://aka.ms/vs2017componentid-binaries) します。 スプレッドシートの使用方法の詳細については、「[方法: 機能拡張プロジェクトを Visual Studio 2017 に移行](how-to-migrate-extensibility-projects-to-visual-studio-2017.md)する」の「**コンポーネント id の検索**」セクションを参照してください。
...
`PackageRegistrationAttribute`を使用する場合は、これが自動的に行われます。
...
［はい、 Visual Studio 2015 で VSIX v3 形式のマニフェストを構築するために必要なツールとタスクが含まれている NuGet パッケージがあります。 機能拡張プロジェクトに NuGet パッケージ `Microsoft.VisualStudio.Sdk.BuildTasks.14.0` への参照を追加します。 また、`v3` を指定する `VsixType` 要素をプロジェクトファイルに追加する必要があります。
...
いいえ、VSIX v3 の `InstallRoot` プロパティを使用しても、Visual Studio のインストールフォルダー構造の外部にインストールすることはできません。 サポートされている場所については、「[拡張機能フォルダー以外のインストール](set-install-root.md)」を参照してください。
