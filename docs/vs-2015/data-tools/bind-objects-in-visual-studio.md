...
> [!NOTE]
> Visual Studio は TableAdapter にクエリ `Fill` を実行します `GetData` 。既定では、これらの名前は任意の有効なメソッド名に変更できます。
...
### <a name="addobjects-to-a-collection"></a>コレクションへの addobjects
 ...
 を使用してコレクションに追加する例につい <xref:System.Windows.Forms.BindingSource> ては、「 `LoadCustomers` [チュートリアル: オブジェクト内のデータへの接続 (Windows フォーム)](https://msdn.microsoft.com/library/21a7fba2-b38b-4726-8cbe-d22154b75a05)」のメソッドを参照してください。

 カスタムコレクションにオブジェクトを追加する例については、「 `LoadOrders` [チュートリアル: オブジェクトのデータへの接続 (Windows フォーム)](https://msdn.microsoft.com/library/21a7fba2-b38b-4726-8cbe-d22154b75a05)」のメソッドを参照してください。
...
> [!NOTE]
> この例で `Orders` は、コレクションはオブジェクトのプロパティです `Customer` 。
