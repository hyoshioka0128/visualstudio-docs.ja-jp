...
|プロパティ|説明|予約または既知|  
|--------------|-----------------|-----------------------------|  
|`MSBuildBinPath`|現在使用されているバイナリが配置されているフォルダーの絶対パス [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] (たとえば、C:\windows\ microsoft.net \framework \\ *versionNumber*)。 このプロパティは、[!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] ディレクトリのファイルを参照する必要がある場合に便利です。<br /><br /> このプロパティに最後の円記号を含めないでください。|予約されています。|  
...
|`MSBuildLastTaskResult`|前のタスクがエラーを発生することなく完了した場合は、(警告があった場合でも) `true` を返します。前のタスクでエラーが発生した場合は、`false` を返します。 通常、エラーがタスクで発生する場合、プロジェクト内ではエラーは最後に発生します。 したがって、このプロパティの値は、次のシナリオ以外では `false` にはなりません。<br /><br /> - `ContinueOnError` [Task 要素 (MSBuild)](../msbuild/task-element-msbuild.md) の属性が `WarnAndContinue` (または `true` ) またはに設定されている場合 `ErrorAndContinue` 。<br /><br /> -に `Target` [OnError 要素 (MSBuild)](../msbuild/onerror-element-msbuild.md) が子要素として含まれている場合。|予約されています。|  
|`MSBuildNodeCount`|ビルド時に使用する同時実行プロセスの最大数。 これは、コマンドラインで **/maxcpucount** に指定した値です。 値を指定せずに **/maxcpucount** を指定した場合、は `MSBuildNodeCount` コンピューターのプロセッサ数を指定します。 詳細については、「 [コマンドラインリファレンス](../msbuild/msbuild-command-line-reference.md) 」と「 [複数のプロジェクトの並行ビルド](../msbuild/building-multiple-projects-in-parallel-with-msbuild.md)」を参照してください。|予約済み|  
...
|`MSBuildProjectExtension`|ピリオドを含むプロジェクト ファイルの名前の拡張子.proj のようになります。|予約済み|  
|`MSBuildProjectFile`|ファイル名の拡張子を含むプロジェクト ファイルの完全なファイル名。MyApp.proj のようになります。|予約されています。|  
|`MSBuildProjectFullPath`|ファイル名の拡張子を含む、プロジェクト ファイルの絶対パスと完全なファイル名。C:\MyCompany\MyProduct\MyApp.proj のようになります。|予約済み|  
|`MSBuildProjectName`|ファイル名の拡張子のないプロジェクト ファイルの名前。MyApp のようになります。|予約済み|  
