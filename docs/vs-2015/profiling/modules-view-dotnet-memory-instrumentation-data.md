...
|Column|[説明]|  
|------------|-----------------|  
|**経過時間 (子を含む)**|-   関数の場合、その関数で費やされた時間。 これには、子関数で費やされた時間とオペレーティング システムの呼び出し (コンテキストの切り替え、入出力操作など) で費やされた時間が含まれます。<br />-   モジュールの場合、モジュール内の少なくとも 1 つの関数が呼び出し履歴に存在していた期間。|  
...
|Column|[説明]|  
|------------|-----------------|  
|**アプリケーション時間 (関数のみ)**|-   関数の場合、この関数の呼び出しに費やされたアプリケーション排他時間の合計。<br />-   モジュールの場合、モジュール内の関数のすべての呼び出しのアプリケーション排他時間の合計。|  
