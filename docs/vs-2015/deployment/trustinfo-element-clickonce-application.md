...
## <a name="elements-and-attributes"></a>要素と属性  
 `trustInfo` 要素は必須です。この要素は `asm.v2` 名前空間にあります。 これには、属性はなく次の要素を含みます。  
...
## <a name="ipermission"></a>IPermission  
 省略可能。 この要素は `PermissionSet` 要素の子です。 `IPermission` 要素には、 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]内のアクセス許可クラスを正しく指定します。 `IPermission` 要素には次の属性がありますが、アクセス許可クラスのプロパティに対応する追加の属性を持つことができます。 アクセス許可の具体的な構文については、Security.config ファイル内の例を参照してください。  
...
- `permissionSetReference`  
     ...
     必須です。 既定のアクセス許可として使用するアクセス許可セットの ID を指定します。 アクセス許可セットは、 `PermissionSet` 要素で宣言します。  
...
- `Level`  
     ...
     必須です。 アプリケーションが要求するセキュリティ レベルを指定します。 次のいずれかの値になります。  
...
## <a name="examples"></a>例  
 次の 3 つのコード例では、 `trustInfo` による配置のアプリケーション マニフェストで使用する、既定の名前付きセキュリティ ゾーン (Internet、LocalIntranet、および FullTrust) に対応する [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] 要素を示しています。  
