...
- メニュー バーで、 **[デバッグ]** 、 **[グラフィックス]** 、 **[診断の開始]** の順に選択します。 (キーボード:Alt + F5 キーを押します)  
  
- [ **グラフィックス** ] ツールバーで、[ **診断の開始** ] ボタンをクリックします。  
...
- Visual Studio の [ **グラフィックス** ] ツールバーで、[ **フレームのキャプチャ** ] ボタン [![グラフィックスキャプチャ] ボタンアイコン](../debugger/media/debuggingdirectxgraphics.png "DebuggingDirectXGraphics")をクリックします。  
...
- Visual Studio のキャプチャインターフェイスで、**診断セッション**のタイムラインの上にある [**フレームのキャプチャ**] ボタンを選択するか、[ **1 秒あたりのフレーム数**] スイムレーンの下、またはキャプチャされたフレームの右側にある [大きい**キャプチャフレーム**] ボタンを選択します。 下の画像では、両方のボタンが強調表示されています。  
  
   ![GPU 使用率ツールを使用してフレームをキャプチャする。](../debugger/media/pix-gpu-usage-tool-capture-frame.png "pix_gpu_usage_tool_capture_frame")  
  
   キャプチャしたフレームを確認する準備ができたら、**フレーム**に従って**Visual Studio Graphics Analyzer**を開始します。 画像のサムネイルの上またはサムネイルをダブルクリックして、リンクを表示します。  
...
## <a name="capturing-graphics-information-remotely"></a>グラフィックス情報をリモートでキャプチャする  
 ...
 プロジェクトをリモート デバッグ用に構成する方法は、開発中のアプリの種類と、使用しているプログラミング言語によって異なります。 Windows ストアアプリのリモートデバッグを構成する方法の詳細については、「 [リモートコンピューターでの Windows ストアアプリの実行](../debugger/run-windows-store-apps-on-a-remote-machine.md)」を参照してください。 Windows デスクトップアプリのリモートデバッグを構成する方法の詳細については、「 [Visual Studio プロジェクトのリモートデバッグを設定](https://msdn.microsoft.com/library/ec332dc4-400a-498b-a0e6-c8dcf10fef8a)する」を参照してください。  
  
 後で、情報をキャプチャした場所にかかわらず、リモート コンピューターまたはリモート デバイスを使用してグラフィックス情報を再生できます。 詳細については、[グラフィックス診断再生マシンを変更する](../debugger/how-to-change-the-graphics-diagnostics-playback-machine.md)」を参照してください。  
