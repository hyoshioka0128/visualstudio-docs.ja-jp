...
- 必要なタイミング インストルメンテーションをサポートする GPU とドライバー。  
  ...
  グラフィックス診断の要件の詳細については、「 [はじめに](../debugger/getting-started-with-visual-studio-graphics-diagnostics.md)」を参照してください。  
...
2. レポートの上部で、調査対象の問題を示している範囲をいずれか 1 つのグラフから選択します。 選択できる範囲は最大 3 秒間までです。それより長い範囲は、先頭に向かって切り捨てられます。  
    ...
    ![&#45;コレクションに投稿し、詳細を表示する範囲を選択します](../debugger/media/gfx-diag-gpu-usage-select1.png "gfx_diag_gpu_usage_select1")  
  
3. レポートの下部にある [ **詳細の表示** ] リンクを選択します **。ここをクリックすると、その範囲の GPU 使用率の詳細が表示** され、選択したタイムラインの詳細が表示されます。  
  
    ![&#45;コレクション、範囲が選択された投稿](../debugger/media/gfx-diag-gpu-usage-select2.png "gfx_diag_gpu_usage_select2")  
