...
```  
 ...
 `ASSERT` 式はプログラムのリリース バージョンでは評価されないため、`nM` の値は、デバッグ バージョンとリリース バージョンとでは異なります。 MFC でこの問題を回避するには、`ASSERT` ではなく [VERIFY](https://msdn.microsoft.com/library/3e1ab4ee-cbc7-4290-a777-c92f42ce7b96) マクロを使用します。  `VERIFY` によってすべてのバージョンの式が評価されますが、リリース バージョンの結果はチェックされません。  
