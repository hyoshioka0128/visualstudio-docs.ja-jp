...
## <a name="BKMK_Enable_or_disable_Just_My_Code"></a>"マイ コードのみ" の有効/無効の切り替え  
 ...
 ![[オプション] ダイアログボックスでマイコードのみを有効にする](../debugger/media/dbg-justmycode-options.png "DBG_JustMyCode_Options")  
...
## <a name="BKMK__NET_Framework_Just_My_Code"></a>.NET Framework マイコードのみ  
  
### <a name="BKMK_NET_User_and_non_user_code"></a>ユーザーコードと非ユーザーコード  
...
### <a name="BKMK_CPP_User_and_non_user_code"></a>ユーザーコードと非ユーザーコード  
...
- `*.natjmc` フォルダーの `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` ファイルに指定された関数。  
  
  **反復**  
  
  既定では、`*.natstepfilter` フォルダーの `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` ファイルに指定された関数だけが非ユーザー コードと見なされます。  
  
  `.natstepfilter` に独自の `.natjmc` および `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers` を作成して、ステップ実行と呼び出し履歴ウィンドウの動作をカスタマイズすることができます。  
...
### <a name="BKMK_CPP_Customize_stepping_behavior"></a>ステップ実行動作のカスタマイズ  
 `*.natstepfilter` ファイルに関数を非ユーザー コードとして記述することで、それらの関数をステップ オーバーすることを指定できます。  
  
- Visual Studio コンピューターのすべてのユーザーに対して非ユーザーコードを指定するには、`%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` フォルダーに .natstepfilter ファイルを追加します。  
  
- 個々のユーザーに対して非ユーザーコードを指定するには、`%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers` フォルダーに .natstepfilter ファイルを追加します。  
...
|`Name`|必須。 一致を照合する完全な関数名を指定する ECMA-262 書式の正規表現。 例 :<br /><br /> `<Name>MyNS::MyClass.*</Name>`<br /><br /> は、`MyNS::MyClass` のすべてのメソッドが非ユーザー コードと見なされることをデバッガーに知らせます。 照合では大文字と小文字が区別されます。|  
|`Module`|省略可。 関数を含むモジュールへの完全パスを指定する ECMA-262 書式の正規表現。 一致では、大文字と小文字を区別しません。|  
|`Action`|必須。 大文字と小文字が区別される以下のいずれかの値です。<br /><br /> -   `NoStepInto` –一致した関数をステップオーバーするようデバッガーに指示します。<br />-   `StepInto` –一致した関数にステップインし、一致した関数の他の `NoStepInto` をオーバーライドするようにデバッガーに指示します。|  
...
- Visual Studio コンピューターのすべてのユーザーに対して非ユーザーコードを指定するには、`%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` フォルダーに .natjmc ファイルを追加します。  
  
- 個々のユーザーに対して非ユーザーコードを指定するには、`%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers` フォルダーに .natjmc ファイルを追加します。  
...
|`Company`|省略可。 実行可能ファイルに埋め込まれているモジュールを発行する会社の名前。 この属性を使用して、モジュールのあいまいさを解消することができます。|  
...
|`Module`|省略可。 関数を含むモジュールの名前または完全パス。 この属性を使用して、同じ名前の関数のあいまいさを解消することができます。|  
...
### <a name="BKMK_JS_User_and_non_user_code"></a>ユーザーコードと非ユーザーコード  
 **コードの分類**  
...
- ホストから提供された `eval` 関数に文字列を渡すことによって実行されるスクリプトは、 **Mycode**として分類されます。  
  
- 文字列を `Function` コンストラクターに渡すことによって実行されるスクリプトは、 **Librarycode**として分類されます。  
  
- WinJS や Azure SDK などのフレームワーク参照に含まれるスクリプトは、 **Librarycode**として分類されます。  
  
- `setTimeout`、`setImmediate`、または `setInterval` 関数に文字列を渡すことによって実行されるスクリプトは、非関連性のある**コード**として分類されます。  
...
### <a name="BKMK_JS_Customize_Just_My_Code"></a>マイコードのみのカスタマイズ  
