---
title: ウォッチウィンドウとクイックウォッチウィンドウ |Microsoft Docs
ms.date: 11/15/2016
...
ms.locfileid: "90841352"
---
# <a name="watch-and-quickwatch-windows"></a>ウォッチ ウィンドウと [クイック ウォッチ] ウィンドウ
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

[ **ウォッチ** ] ([デバッグ]/[ウィンドウ]/[**ウォッチ]/[ウォッチ (1, 2, 3, 4)**)] と [ **クイックウォッチ** ] ([変数]/[デバッグ]/[ **クイックウォッチ**]) ウィンドウを使用すると、デバッグセッション中に変数と式を監視できます。  この 2 つのウィンドウの違いは、 **ウォッチ** ウィンドウでは複数の変数を表示できますが、 **[クイック ウォッチ]** ウィンドウでは一度に 1 つの変数しか表示できないことです。  
...
2. **F5 キーを押し**てデバッグを開始します。 ブレークポイントで実行が停止します。  
...
## <a name="refreshing-watch-values-that-are-out-of-date"></a><a name="bkmk_refreshWatch"></a> 古いウォッチ値の更新  
...
- •式には、アプリケーションで副作用を引き起こす可能性のある関数呼び出しが含まれています (「 [副作用と式」を](#bkmk_sideEffects)参照してください)。  
...
## <a name="side-effects-and-expressions"></a><a name="bkmk_sideEffects"></a> 副作用と式  
...
```  
 ...
 プロパティの評価または暗黙的な関数呼び出しがオフに設定されている場合でも、 **ac** 書式修飾子を使用して、強制的に評価できます (C# の場合のみ)。 「 [C# での書式指定子」を](../debugger/format-specifiers-in-csharp.md)参照してください。  
...
## <a name="using-registers-in-the-watch-window-c-only"></a>ウォッチ ウィンドウでのレジスタの使用 (C++ のみ)  
 ネイティブコードをデバッグしている場合は、またはを使用して、レジスタ名と変数名を追加でき **$\<register name>** **@\<register name>** ます。  詳細については、「 [Pseudovariables](../debugger/pseudovariables.md)」を参照してください。  
...
## <a name="dynamicview-and-the-watch-window"></a>動的ビューとウォッチ ウィンドウ  
 ...
 ウォッチウィンドウがを実装する型から作成されたオブジェクトまたはオブジェクトを表示すると、デバッガーによって、[ <xref:System.Dynamic.IDynamicMetaObjectProvider> **自動変数**] 表示に特殊な**動的ビュー**ノードが追加されます。 このノードには動的オブジェクトの動的メンバーが表示されますが、そのメンバーの値を編集することはできません。  
  
 **動的ビュー** の任意の子を右クリックして **[ウォッチ式の追加]** を選択すると、デバッガーはオブジェクトを動的オブジェクトにキャストする新しいウォッチ変数を挿入します。 つまり、 **オブジェクト名** が **((動的) オブジェクト).名前**になります。  
  
 **動的ビュー**のメンバーを評価すると、副作用が生じる可能性があります。 副作用の詳細については、「 [Side Effects and Expressions](#bkmk_sideEffects)」を参照してください。 C# の場合、コード行の新しいステップに移行したときに、 **動的ビュー** に表示される値をデバッガーが自動的に再評価することはありません。 Visual Basic の場合、 **動的ビュー** を通じて追加された式は自動的に最新の情報に更新されます。  
