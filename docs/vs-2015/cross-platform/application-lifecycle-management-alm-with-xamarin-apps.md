...
|特徴量|Xamarin でサポートされているかどうか|その他のコメント|  
|-------------|----------------------------|-------------------------|  
|シーケンス図|はい||  
|依存関係グラフ|はい||  
|呼び出し階層|はい||  
...
|特徴量|Xamarin でサポートされているかどうか|その他のコメント|  
|-------------|----------------------------|-------------------------|  
|オンプレミス TFS サーバー|はい|ビルド コンピューターに Xamarin がインストールされている必要があります。iOS 用にビルドするには、OSX コンピューターにリンクできる必要があります。 「 [Xamarin 用の TFS の構成](/azure/devops/repos/tfvc/overview?view=azure-devops) (xamarin web サイト)」を参照してください。|  
...
|コード カバレッジ|N/A||  
|[コードの単体テスト](../test/unit-test-your-code.md)|はい|Windows と Android を対象にする場合は、組み込みの MSTest ツールを使用できます。 Windows、Android、および iOS で単体テストを実行するには、Xamarin では NUnit が推奨されています。 「 [Xamarin 用に TFS を構成する](/azure/devops/repos/tfvc/overview?view=azure-devops) 」(Xamarin web サイト) を参照してください。|  
