...
警告 C6279: \<variable> スカラー new を使用して割り当てられています。 array delete [] を使用して削除されました  
 ...
 この警告は、C++ コードでのみ表示され、呼び出し元の関数がスカラー **new** 演算子を使用したメモリ割り当ての一貫性がないことを示していますが、array **delete []** 演算子を使用して解放されています。 メモリがスカラーの **new**で割り当てられている場合は、通常、スカラーの **delete**を使用して解放する必要があります。  
