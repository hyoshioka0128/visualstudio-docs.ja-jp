...
警告 C6216: 意味の異なる整数型の間でのコンパイラ挿入されたキャストです: ブール型から HRESULT  
 ...
 この警告は、ブール値が `HRESULT` 明示的にキャストされることなく、として使用されていることを示します。 ブール型は 0 以外の値で成功を示します。`S_OK` の成功値 (`HRESULT`) は値 0 によって示されます。 ブール値 false を返す関数の一般的なエラー値は、としてテストされた場合の成功状態です `HRESULT` 。 これは、正しくない結果につながる可能性があります。  
