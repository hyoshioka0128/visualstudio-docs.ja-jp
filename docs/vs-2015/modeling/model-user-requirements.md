...
|-------------------------|-----------------------------------------------|-------------|
|ユース ケース図|だれがシステムを使用し、ユーザーがシステムを使って何をするか。|[システムの使用方法の記述](#UseCases)|
|概念クラス図|要求の記述に使用する型の用語集。システムのインターフェイスで確認できる型。|[要求の記述に使われる用語を定義する](#RequirementsClasses)|
...
|追加のドキュメントまたは作業項目|特定のユース ケースに固有でない制約とルール|[ビジネスルールの表示](#BusinessRules)|
...
## <a name="describing-how-your-system-is-used"></a><a name="UseCases"></a> システムの使用方法の説明
 ...
 !["顧客" と "レストラン" のユース ケース](../modeling/media/uml-reqmuc1.png "UML_ReqmUC1")

 また、ユース ケースがより小さいケースで構成されていることを示すこともできます。 たとえば料理の注文は、料理の購入の一部ですが、購入には支払いと配達も含まれています。

 ![システムは支払いに関与するが納入には関与しない](../modeling/media/uml-reqmuc2.png "UML_ReqmUC2")
...
## <a name="defining-terms-used-to-describe-requirements"></a><a name="RequirementsClasses"></a> 要件の記述に使用される用語の定義
...
- システムまたは受け入れテストの説明。
  ...
  ![クラス メニュー、注文、メニュー項目、注文項目](../modeling/media/uml-reqmcd1.png "UML_ReqMCD1")
...
## <a name="showing-business-rules"></a><a name="BusinessRules"></a> ビジネスルールの表示
 ビジネス ルールとは、特定のユース ケースに関連付けられておらず、システム全体で認められる要求です。

 多くのビジネス ルールは、概念クラス間の関係に関する制約です。 これらの *静的ビジネスルール* は、概念クラス図の関連クラスに関連付けられたコメントとして記述できます。 たとえば、次のように入力します。

 ![Order クラスにアタッチされたコメント内のルール](../modeling/media/uml-reqmcd2.png "UML_ReqmCD2")
...
## <a name="describing-quality-of-service-requirements"></a><a name="QoSRequirements"></a> サービス品質要求の記述
...
## <a name="showing-work-flow-between-users-and-your-system"></a><a name="Workflow"></a> ユーザーとシステム間のワークフローを表示する
 アクティビティ図を使って、さまざまなユース ケース間のワークフローを表示することができます。 多くの場合、ユーザーがシステムおよびシステム外の両方で実行する主なタスクを示すアクティビティ図を描画して、要件のモデルを開始すると便利です。

 たとえば、次のように入力します。

 ![3 つのアクションと 1 つのループがあるアクティビティ](../modeling/media/uc-reqmwfact.png "UC_ReqmWFAct")

 ユース ケース図やアクティビティ図を描画して、同じ情報を異なるビューで表示することができます。  ユース ケース図は、より大規模なアクティビティ内の小さいアクションの入れ子構造を表示するのにより効果的ですが、ワークフローは表示されません。 たとえば、次のように入力します。
...
## <a name="showing-interactions-between-users-and-your-system"></a><a name="Sequences"></a> ユーザーとシステム間の相互作用の表示
 ...
 ![システムとアクターを含むシーケンス ダイアグラム](../modeling/media/uml-reqmseq.png "UML_ReqmSeq")
