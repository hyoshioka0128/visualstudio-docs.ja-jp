...
ツールボックスから、または貼り付け操作または移動操作で、要素を別の要素にドラッグすることを許可できます。 指定したリレーションシップを使用して、移動された要素をターゲット要素にリンクすることができます。
 ...
 要素マージディレクティブ (EMD) は、1つのモデル要素が別のモデル要素に *マージ* された場合の動作を指定します。 これは次の場合に発生します。
...
>
> 新しい要素または新しいリレーションシップが必ずカスタムコードによって処理されるようにするには、 `AddRule` 埋め込みリレーションシップでを定義し、を `DeleteRule` 要素のドメインクラスに定義することを検討してください。 詳細については、「 [ルールによってモデル内の変更が反映される](../modeling/rules-propagate-changes-within-the-model.md)」を参照してください。
...
2. ユーザーが要素を図形にマージできるようにするには `ExampleElement` 、ドメインクラスで新しい EMD を作成し `ExampleElement` ます。

   1. **DSL エクスプローラー**で、[**ドメインクラス**] を展開します。 を右クリック `ExampleElement` し、[ **新しい要素のマージディレクティブの追加**] をクリックします。

   2. 新しい EMD の詳細を確認できるように、 **DSL の詳細** ウィンドウが開いていることを確認します。 (メニュー: **表示**、 **その他のウィンドウ**、 **DSL の詳細**。)

3. [DSL の詳細] ウィンドウで **インデックス作成クラス** を設定して、オブジェクトにマージできる要素のクラスを定義し `ExampleElement` ます。

    この例では、を選択して、 `ExampleElements` ユーザーが新しい要素を既存の要素にドラッグできるようにします。
...
4. [ **リンクを作成してマージを処理する**] で、次の2つのパスを追加します。
   ...
   3. [ **パスにリンクを作成してマージを処理する**] で、[] をクリックし **\<add path>** ます。
...
5. DSL をテストします。
   ...
   2. の実験用インスタンス [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] が起動したら、DSL のモデルファイルを開きます。 いくつかの例の要素を作成します。
...
- EMD は、ツールボックスからの作成と貼り付け操作の両方に影響します。
     ...
     新しい要素を作成するカスタムコードを記述する場合は、メソッドを使用して明示的に EMD を呼び出すことができ `ElementOperations.Merge` ます。 これにより、コードは、他の操作と同じ方法で新しい要素をモデルにリンクします。 詳細については、「 [コピー動作のカスタマイズ](../modeling/customizing-copy-behavior.md)」を参照してください。
...
2. DSL エクスプローラーで、[ **ドメインクラス**]、 `ExampleModel` [ **要素マージディレクティブ**] の順に展開します。 という名前の要素マージディレクティブを選択し `ExampleElement` ます。

     この EMD は、ユーザーが [ `ExampleElement` ツールボックス] からドラッグして、モデル内に新しいオブジェクトを作成する方法を制御します。
...
4. ソリューションをリビルドします。 生成されたコードはモデルから更新されるため、この処理には通常より長い時間がかかります。
     ...
     メソッドを実装する必要があり `CanMergeExampleElement` ます。
...
5. **Dsl**プロジェクトに新しいコードファイルを作成します。 その内容を次のコードに置き換えて、名前空間をプロジェクトの名前空間に変更します。
    ...
    ```
     ...
     この単純な例では、親モデルにマージできる要素の数を制限しています。 より興味深い条件では、メソッドは、受信側オブジェクトのすべてのプロパティとリンクを検査できます。 また、に格納されているマージ要素のプロパティを検査することもでき <xref:Microsoft.VisualStudio.Modeling.ElementGroupPrototype> ます。 の詳細について `ElementGroupPrototypes` は、「 [コピー動作のカスタマイズ](../modeling/customizing-copy-behavior.md)」を参照してください。 モデルを読み取るコードを記述する方法の詳細については、「 [プログラムコードでのモデルのナビゲーションと更新](../modeling/navigating-and-updating-a-model-in-program-code.md)」を参照してください。

6. DSL をテストします。

    1. F5 キーを押してソリューションをリビルドします。 の実験用インスタンスが開いたら [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 、DSL のインスタンスを開きます。
...
2. メソッドをオーバーライドし、必要に応じてメソッドをオーバーライドし `MergeRelate` `MergeDisconnect` ます。 これを行うには、ドメインクラスの [ **Double の派生** プロパティを生成する] を設定する必要があります。 コードでは、基底クラスで生成されたマージコードを呼び出すことができます。 マージの実行後に追加の操作を実行する場合は、このオプションを使用します。

   これらの方法は、この EMD を使用して実行されるマージにのみ影響します。 マージされた要素を作成するすべての方法に影響を与える場合は、埋め込みリレーションシップでを定義する方法と、マージされたドメインクラスにを定義する方法があり `AddRule` `DeleteRule` ます。 詳細については、「 [ルールによってモデル内の変更が反映される](../modeling/rules-propagate-changes-within-the-model.md)」を参照してください。
...
2. DslDefinition ダイアグラムで、マージの受信クラスを選択します。 通常は、埋め込みリレーションシップのソースエンドのクラスです。
     ...
     たとえば、最小言語ソリューションから生成された DSL では、を選択し `ExampleModel` ます。
...
6. 新しいコードファイルで、受信側クラスの部分クラスを記述し、メソッドをオーバーライドし `MergeRelate` ます。 基本メソッドを必ず呼び出してください。 たとえば、次のように入力します。
...
1. **生成された Code:\ cs**で、という名前のメソッドを検査 `MergeRelate` します。 これらのメソッドは、新しい要素と既存のモデル間のリンクを作成します。

    また、という名前のメソッドを検査 `MergeDisconnect` します。 これらのメソッドは、要素を削除するときに、その要素をモデルからリンク解除します。
...
3. ソリューションをリビルドします。 生成されたコードファイルがモデルから更新されるため、通常よりも時間がかかります。
    ...
    エラーメッセージが表示されます。 エラーメッセージをダブルクリックすると、生成されたコードの指示が表示されます。 次の手順では、 `MergeRelate` *domainclass*と `MergeDisconnect` *domainclass*の2つのメソッドを指定するように求められます。

4. 部分クラス定義内のメソッドを別のコードファイルに記述します。 前に検査した例では、必要なものを提案します。

   カスタムマージコードは、オブジェクトとリレーションシップを直接作成するコードには影響しません。また、他の変更には影響しません。 要素の作成方法に関係なく、追加の変更が実装されていることを確認するには、とを作成することを検討してください `AddRule` `DeleteRule` 。 詳細については、「 [ルールによってモデル内の変更が反映される](../modeling/rules-propagate-changes-within-the-model.md)」を参照してください。
...
1. [!INCLUDE[dsl](../includes/dsl-md.md)]コンポーネントモデルテンプレートを使用して、ソリューションを作成します。
...
10. ソリューションをビルドして実行します。 の新しいインスタンス [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] が表示されます。
...
## <a name="see-also"></a>参照
 [プログラム内のモデルの移動と更新コード](../modeling/navigating-and-updating-a-model-in-program-code.md)[ツールとツールボックスのカスタマイズ](../modeling/customizing-tools-and-the-toolbox.md)
