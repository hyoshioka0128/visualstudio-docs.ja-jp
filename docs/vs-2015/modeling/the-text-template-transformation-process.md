...
|Host|ホストは、エンジンとユーザー環境の間のインターフェイスです。 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] は、テキスト変換プロセスのホストです。|はい。 カスタムホストを作成することができます。|
...
- テキストテンプレートの変換エラーを処理しています。 たとえば、ホストは、ユーザーインターフェイスのエラーを表示したり、ファイルに書き込んだりできます。 (では [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 、エラーはエラーメッセージウィンドウに表示されます)。
