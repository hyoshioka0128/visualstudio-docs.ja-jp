...
Windows フォームを使用すると、DSL 図を使用する代わりに、ドメイン固有言語 (DSL) モデルの状態を表示できます。 このトピックでは、視覚化およびモデリング SDK を使用して、Windows フォームを DSL にバインドする手順について説明し [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ます。
...
2. テンプレートに用意されている最初の例を試してみてください。
   ...
   のメインインスタンスでは、 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] DSL ソリューションに関する次の点に注意してください。
