...
- [図内でのテキスト検索](#Searching)

- [プレゼンテーション用の図の準備](#presentation)

- [UML デザイナーの拡張](#extensions)

## <a name="uml-diagrams-are-views-of-a-uml-model"></a><a name="Views"></a> Uml 図は UML モデルのビューです
...
- いずれかの図を保存すると、モデル全体、そのすべての図、およびモデリング プロジェクト ファイルが Visual Studio によって保存されます。

## <a name="creating-uml-modeling-diagrams"></a><a name="Creating"></a> UML モデリング図の作成
...
## <a name="drawing-uml-modeling-diagrams"></a><a name="Drawing"></a> UML モデリング図の描画
...
## <a name="editing-shapes-and-connectors"></a><a name="Editing"></a> 図形とコネクタの編集
...
- **アーキテクチャデザイナー**のツールバーで、プラス記号 ( **+** ) またはマイナス記号 ( **-** ) をクリックするか、ズームレベルを選択します。

## <a name="searching-in-a-diagram"></a><a name="Searching"></a> ダイアグラム内の検索
...
## <a name="undoing-changes-to-the-model"></a><a name="Undo"></a> モデルに対する変更を元に戻す
 ...
 **図を閉じると変更のスタックが切り捨てられます。** 図を閉じると、その図に対して実行した変更は元に戻せなくなります。また、それ以前にモデルまたはその図に対して加えた変更も元に戻せなくなります。
 ...
 **図を保存せずに閉じると、意図しない結果になる場合があります。** 図に変更を加えた後にその図を保存せずに閉じても、変更内容はモデルで保持されます。 図を保存せずに閉じる必要がある場合は、モデル全体を閉じることをお勧めします。

## <a name="sharing-elements-between-diagrams"></a><a name="Sharing"></a> ダイアグラム間での要素の共有
...
## <a name="copying-elements-and-groups-of-related-elements"></a><a name="Copying"></a> 要素と関連要素のグループのコピー
...
## <a name="deleting-a-model-element-or-its-views"></a><a name="Deleting"></a> モデル要素またはそのビューの削除
...
## <a name="preparing-a-diagram-for-presentation"></a><a name="presentation"></a> 図をプレゼンテーション用に準備する
...
## <a name="extending-the-uml-designers"></a><a name="extensions"></a> UML デザイナーの拡張
