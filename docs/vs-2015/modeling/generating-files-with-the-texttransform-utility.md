...
> [!NOTE]
> ビルド処理の一部としてテキスト変換を実行する場合は、MSBuild テキスト変換タスクの使用を検討してください。 詳細については、「 [ビルドプロセスでのコード生成](../modeling/code-generation-in-a-build-process.md)」を参照してください。 がインストールされているコンピューターでは [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 、テキストテンプレートを変換できるアプリケーションまたは拡張機能を作成することもでき [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ます。 詳細については、「 [カスタムホストを使用したテキストテンプレートの処理](../modeling/processing-text-templates-by-using-a-custom-host.md)」を参照してください。
