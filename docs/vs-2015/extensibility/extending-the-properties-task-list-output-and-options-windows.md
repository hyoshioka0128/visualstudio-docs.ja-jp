---
title: '[プロパティ]、[タスク一覧]、[出力]、[オプション] の各ウィンドウを拡張するMicrosoft Docs'
ms.date: 11/15/2016
...
1. TodoWindowControl.xaml.cs ファイルで、次の using ステートメントを追加します。  
...
1. `ToolsOptions.cs`という名前のクラスを追加します。 ToolsOptions クラスがから継承されるように <xref:Microsoft.VisualStudio.Shell.DialogPage> します。  
...
1. TodoWindowPackage.cs で、 <xref:Microsoft.VisualStudio.Shell.ProvideOptionPageAttribute> TodoWindowPackage クラスにを追加します。  
  
    ```csharp  
    [ProvideOptionPage(typeof(ToolsOptions), "ToDo", "General", 101, 106, true)]  
    ```  
  
2. 指定されたクラスコンストラクターの最初のパラメーターは、前に作成した ToolsOptions クラスの型です。 2番目のパラメーター "ToDo" は、[ **オプション** ] ダイアログボックスのカテゴリの名前です。 3番目のパラメーター "General" は、オプションページを使用できるようにする [ **オプション** ] ダイアログボックスのサブカテゴリの名前です。 次の2つのパラメーターは、文字列のリソース Id です。1つ目はカテゴリの名前で、2つ目はサブカテゴリの名前です。 最後のパラメーターは、オートメーションを使用してこのページにアクセスできるかどうかを決定します。  
...
## <a name="make-data-available-to-the-properties-window"></a>[プロパティ] ウィンドウでデータを使用できるようにする  
 ToDo リストの各項目に関する情報を格納する TodoItem という名前のクラスを作成することによって、使用可能な To Do list 情報を作成できます。  
...
2. 次の using ステートメントを TodoItem.cs ファイルに追加します。  
...
3. `public`アクセス修飾子をクラス宣言に追加します。  
    ...
    ```  
     ...
     Name と DueDate という2つのプロパティを追加します。 UpdateList () と CheckForErrors () を後で実行します。  
...
4. ユーザーコントロールにプライベート参照を追加します。 ユーザーコントロールとこの ToDo 項目の名前を受け取るコンストラクターを追加します。 DaysAhead の値を検索するには、[オプション] ページのプロパティを取得します。  
...
6. TodoWindowControl.xaml.cs で、メソッドとメソッドの TodoWindowControl クラスにスタブメソッドを追加し `CheckForError` `UpdateList` ます。 これらの文字列は、Processの文字の後、ファイルの末尾の前に配置します。  
...
1. 既存の `button1_Click` 関数を新しい TodoItem を作成するコードに置き換え、それをリストボックスに追加します。 TrackSelection () を呼び出します。これは後で定義されます。  
...
3. TrackSelection () メソッドを実装します。 サービスを取得する必要があるため、 <xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell> <xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection> TodoWindowControl からアクセスできるようにする必要があり <xref:Microsoft.VisualStudio.Shell.WindowPane.GetService%2A> ます。 TodoWindow クラスに次のメソッドを追加します。  
...
4. 次の using ステートメントを TodoWindowControl.xaml.cs に追加します。  
...
## <a name="add-text-to-the-output-window-and-items-to-the-task-list"></a>出力ウィンドウと項目にテキストをタスク一覧に追加します  
 **タスク一覧**には、task 型の新しいオブジェクトを作成し、add メソッドを呼び出してそのタスクオブジェクトを**タスク一覧**に追加します。 **出力**ウィンドウに書き込むには、getpane メソッドを呼び出して pane オブジェクトを取得した後、pane オブジェクトの outputstring メソッドを呼び出します。  
...
2. タスク一覧に項目を追加するには、TodoWindowControl クラスに入れ子になったクラスを追加するためのが必要です。 入れ子になったクラスは、から派生する必要があり <xref:Microsoft.VisualStudio.Shell.TaskProvider> ます。 TodoWindowControl クラスの末尾に次のコードを追加します。  
...
3. 次に、TodoTaskProvider へのプライベート参照と CreateProvider () メソッドを TodoWindowControl クラスに追加します。 コードは、次のようになります。  
...
4. ClearError () を追加します。これにより、タスク一覧がクリアされ、ReportError () が TodoWindowControl クラスにタスク一覧エントリを追加します。  
...
2. TodoWindow (**ビュー/その他のウィンドウ/TodoWindow**) を開きます。  
