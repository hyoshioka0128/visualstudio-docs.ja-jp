...
5. コード言語固有のテキストを追加する: `<LanguageSpecificText devLangcs="CS" devLangvb="VB" devLangcpp="C++" devLangnu="F#" />` devLangnu = では、他の言語を入力することができます。 たとえば、コードスニペット DisplayLanguage = Fortran の場合、devLangnu = "Fortran" は Fortran を表示します。
...
2. Help 2.1 レジストリキーの下に、partner 名前空間を登録します。
   ...
   - 32ビットオペレーティングシステムの場合:
      ...
      HKEY_LOCAL_MACHINE \SOFTWARE\Microsoft\Help\v2.1\Partner<em> \\<名前 \> 空間</em>

      "場所" = "オフライン"

   - 64ビットオペレーティングシステムの場合:

      HKEY_LOCAL_MACHINE \SOFTWARE\Wow6432Node\Microsoft\Help\v2.1\Partner<em> \\<名前 \> 空間</em>
...
- METHOD: これは、を通じて渡されるキーワードになります。
  ...
  **IDE でヘルプデバッグツールを有効にする**
  ...
  HKEY_CURRENT_USER \Software\Microsoft\VisualStudio\12.0\Dynamic のヘルプキー: 小売の値にデバッグ出力を表示する: はい
...
- フォールバックブランディングパッケージ (Branding_) は、ヘルプビューアーの \<locale> 言語パックによって、ヘルプビューアー2.1 アプリのルート (例: C:\Program files (x86) \Microsoft Help Viewer\v2.1) にインストールされます。  これは、製品ブランディングパッケージがインストールされていない場合 (コンテンツがインストールされていない場合)、またはインストールされているブランド化パッケージが破損している場合に使用されます。  アプリのルートフォールバックブランドパッケージが使用されている場合、Visual Studio の要素 (ロゴとフィードバック) は無視されます。
...
- Branding.js –コンテンツ動作をサポートするスクリプトファイル
...
- または、SelfBranded = false の場合、MSHA で定義された一意のブランド化パッケージがあり、コンテンツがインストールされているときに使用できます。
  ...
  Branding.xml ファイルには、トピックにが含まれている場合に、トピック内の特定の項目を一貫して表示するために使用されるリスト要素が含まれてい \<meta name="Microsoft.Help.SelfBranded" content="false"/> ます。  Branding.xml ファイル内の要素の Visual Studio の一覧を次に示します。  このリストは、独自の製品ブランド化のニーズを満たすために、これらの要素 (ロゴ、フィードバック、著作権など) を変更する ISO シェル採用者向けのテンプレートとして使用することを目的としています。
...
機能: **CollapsibleArea** Use: Expand 折りたたまれるコンテンツコントロールテキスト
...
機能:**CodeSnippet** Use: コードスニペットコントロールテキスト。  注: "非互換性" 領域があるコードスニペットコンテンツは、スペースに変更されます。
...
機能:**フィードバック、フッター、ロゴの**使用: 現在のトピックに関するフィードバックを電子メールで提供するために、お客様にフィードバックコントロールを提供します。  コンテンツの著作権テキスト。  ロゴの定義。
...
機能:**免責事項**: 機械翻訳されたコンテンツのケース固有の免責事項のセット。
...
機能: **Linktable**の使用: オンラインのトピックリンクのサポート
...
機能:**ビデオオーディオコントロール**の使用: ビデオコンテンツの要素とテキストを表示する
...
機能:**コンテンツがインストールされていないコントロール**使用: contentnotinstalled.htm のレンダリングに使用されるテキスト要素 (文字列)
...
機能:**トピックが見つかりませんコントロール**の使用: topicnotfound.htm のレンダリングに使用されるテキスト要素 (文字列)
...
機能: **Topic 破損**したコントロールの使用: topiccorrupted.htm のレンダリングに使用されるテキスト要素 (文字列)
...
機能:**ホームページコントロール**使用: ヘルプビューアーの最上位レベルのノードコンテンツの表示をサポートするテキスト。
...
- .Css –自己ブランド化が false の場合に表示する css 要素が含まれます。
  ...
  ブランド化された .css ファイルには、Visual Studio トピックプレゼンテーションの定義が含まれています (注意点として、パッケージサービスの .mshc Branding_ に含まれる .css が変更される \<locale> 可能性があります)。
...
### <a name="deploying-a-set-of-topics"></a>一連のトピックを展開する
 ...
 この入門の目的では、MSHA の名前は HelpContentSetup です。 msha (ファイル名は任意で、拡張子は) です。MSHA)。 HelpContentSetup. msha (次の例を参照) には、使用可能な cab または MSHCs の一覧が含まれている必要があります。  ファイルの種類は、MSHA 内で一貫している必要があります (では、MSHA と CAB ファイルの種類の組み合わせはサポートされません)。 CAB または .MSHC ごとに、 \<div class="package"> ...\</div> (次の例を参照してください)。
...
2. [Visual Studio 2013 分離シェル再頒布可能パッケージ](https://aka.ms/VS2013/IsoShell-LP/all)
   ...
   **概要**
   ...
   [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)]シェルは、 [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] アプリケーションのベースとして使用できる IDE のバージョンです。 このようなアプリケーションには、作成した拡張機能と共に分離シェルが含まれています。 拡張機能をビルドするには、SDK に含まれている分離シェルプロジェクトテンプレートを使用し [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] ます。

   分離シェルベースのアプリケーションとそのヘルプを作成するための基本的な手順は次のとおりです。

3. [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)]ISO シェル再頒布可能パッケージ (Microsoft ダウンロード) を入手します。
...
- HKLM\SOFTWARE\Wow6432Node\Microsoft\Help\v2.1\Catalogs\VisualStudio12\en-US
   ...
   キー: CatalogName String 値: [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Documentation
...
9. \\ファイル (x86) にフォルダーを作成 \\ し、という名前を指定し `Contoso` ます。

10. ContosoHelpShell release フォルダーの内容を、\Contoso\ フォルダーにコピーし \\ ます。

11. [**スタート**] メニューの [**実行**] をクリックし、「」と入力して、レジストリエディターを起動し `Regedit` ます。 レジストリエディターで、[**ファイル**]、[**インポート**] の順に選択します。 ContosoHelpShell プロジェクトフォルダーを参照します。 ContosoHelpShell サブフォルダーで、ContosoHelpShell を選択します。

12. コンテンツストアを作成します。

     ISO シェルの場合-Contoso コンテンツストア C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\ContosoDev12 を作成する

     [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)]Integrated Shell の場合は、フォルダー C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\VisualStudio12 を作成します。

13. CatalogType.xml を作成し、次の内容を含むコンテンツストア (前の手順) に追加します。
...
14. 次のレジストリキーを追加します。
     ...
     キー: CatalogName String value: [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Documentation。 ISO シェルの場合、これはカタログの名前です。
