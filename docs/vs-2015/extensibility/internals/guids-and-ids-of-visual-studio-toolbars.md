---
title: ツールバーの Guid と Id |Microsoft Docs
ms.date: 11/15/2016
...
> Visual Studio で使用できるツールバーの多くは、Visual Studio によって定義されていません。 GUID と ID の値はパブリックではありません。 このトピックでは、Visual Studio SDK の vsct ファイルで定義されているツールバーのみを示します。
 ...
 Visual Studio IDE によって提供される既定のツールバーは、guid `guidSHLMainMenu` : ID 構文を使用して指定されている場合を除き、guid を使用します。
...
## <a name="tool-window-toolbars"></a>ツールウィンドウのツールバー
...
## <a name="see-also"></a>参照
 ツール[バーにメニューコントローラーを追加](../../extensibility/adding-a-menu-controller-to-a-toolbar.md)するツールバーをツールウィンドウ[の Guid と Visual Studio メニューの id](../../extensibility/internals/guids-and-ids-of-visual-studio-menus.md) [に追加する](../../extensibility/adding-a-toolbar-to-a-tool-window.md)
