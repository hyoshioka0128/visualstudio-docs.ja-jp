...
新しいプロジェクトの種類を追加する場合は、 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 統合開発環境 (IDE: integrated development environment) でのプロジェクトファイルのオープンと保存を管理する必要があります。 次のトピックでは、ファイルを開いたり保存したりするためのさまざまな方法について説明します。  
