...
以下のセクションでは、で使用できるさまざまな言語サービスオプションのレジストリエントリの一覧を提供し [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] ます。  
  
 次のレジストリエントリの一覧では、 *VS Reg Root*は HKEY_LOCAL_MACHINE \software\microsoft\visualstudio と同じです。ここで、 \\ *X.Y* *x.y*は [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] バージョン番号です。  
...
|名前|Type|Range|説明|  
|----------|----------|-----------|-----------------|  
|(既定)|REG_SZ|テキスト|既定値を使用して、言語の名前を文書化できます。 このキーの名前は、式エバリュエーターの GUID であり、\ ad7metricor 式エバリュエーターに対応するエントリが含まれてい *\<VS Reg Root>* ます。|  
