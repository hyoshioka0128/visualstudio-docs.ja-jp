...
10. Button コントロールを選択します。 プロパティウィンドウで、 **Name** プロパティの値をから \<No Name> button1 に変更します。 次に、**[コンテンツ]** プロパティの値を Button から Start に変更します。

11. ProgressBar コントロールを選択します。 プロパティウィンドウで、 **Name** プロパティの値をから \<No Name> から progressbar1 に変更します。 次に、**[最大値]** プロパティの値を **100** から **10000** に変更します。

12. Checkbox コントロールを選択します。 プロパティウィンドウで、 **Name** プロパティの値をから \<No Name> checkBox1 に変更し、 **IsEnabled** プロパティをクリアします。
...
1. [ **デバッグ** ] メニューの [ **デバッグの開始** ] をクリックするか、 **F5**キーを押します。
...
1. 前に作成した SimpleWPFApp アプリケーションを見つけます。 既定では、アプリケーションは C:\Users \\<username \> \Documents\Visual Studio にあり \<version>\Projects\SimpleWPFApp\SimpleWPFApp\bin\Debug\SimpleWPFApp.exe
...
3. ソリューションエクスプローラーで、ソリューションを右クリックし、[ **追加** ]、[ **新しいプロジェクト**] の順に選択します。
...
8. [UIMap – コード化された UI テスト ビルダー] で **[記録の開始]** を選択します。
     ...
     ![録画を開始して](../test/media/cuit-builder-record.png "CUIT_Builder_Record")
...
13. [UIMap-コード化された UI テストビルダー] で、[ **コードの生成**] を選択します。
...
9. [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]ツールバーの [**保存**] をクリックします。
...
4. テストエクスプローラーで、 **CodedUITestMethod1**を実行します。
