...
## <a name="creating-members"></a><a name="CreateMembers"></a> メンバーの作成
...
2. [クラスの詳細] ウィンドウ ツール バーで、上部のアイコンをクリックし、ドロップダウン リストの **[新しい \<member>]** をクリックします。
...
2. [クラスの詳細] ウィンドウの、追加するメンバーの種類を含むセクションで、 **\<add member>** をクリックします。 たとえば、フィールドを追加する場合は **\<add field>** をクリックします。
...
## <a name="modifying-type-members"></a><a name="ModifyTypeMembers"></a> 型のメンバーの変更
...
- **\<add member>**
     ...
     [クラスの詳細] ウィンドウの最後の行には、 **[名前]** セルに **\<add member>** というテキストが表示されます。 このセルをクリックすると、新しいメンバーを作成できます。 詳細については、「[メンバーの作成](../ide/creating-and-configuring-type-members-class-designer.md#CreateMembers)」を参照してください。
...
2. [クラスの詳細] ウィンドウが表示されていない場合は、クラスデザイナーツールバーの [ **クラスの詳細情報のウィンドウ** ] ボタンをクリックします。
...
## <a name="adding-parameters-to-methods"></a><a name="AddMethodParams"></a> メソッドへのパラメーターの追加
...
- **Type**
   ...
   [ **パラメーターの型** ] セルでは、Intellisense を使用します。これにより、現在のプロジェクトまたは参照先プロジェクトで使用できるすべての型の一覧から選択できます。
...
### <a name="to-add-a-parameter-to-a-method"></a><a name="HowToAddParameterToMethod"></a> パラメーターをメソッドに追加するには
...
2. [クラスの詳細] ウィンドウで、パラメーターを追加するメソッドの行を展開します。
     ...
     インデントを設定したパラメーター行が表示され、かっこのペアと **\<add parameter>** という語だけが表示されます。

3. **\<add parameter>** をクリックし、新しいパラメーターの名前を入力し、**Enter** キーを押します。
...
## <a name="class-details-usage-notes"></a><a name="ClassDetailsUsageNotes"></a> クラスの詳細の使用上の注意
...
## <a name="display-of-read-only-information"></a><a name="ReadOnlyInfo"></a> 読み取り専用の情報の表示
