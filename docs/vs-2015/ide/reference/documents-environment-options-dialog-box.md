 ...
 ** \<** *n* **> その他のファイルプロジェクトに保存され**ている項目は、**ソリューションエクスプローラー**の**MiscellaneousFiles**フォルダーに保持するファイルの数を指定します。 ファイルはエディターで開いていなくても一覧表示されます。 0 から 256 までの任意の整数を指定できます。 既定の数は 0 です。
