...
プロジェクト**デザイナー**の [**参照**] ページを使用すると、プロジェクト内の参照、Web 参照、およびインポートされた名前空間を管理できます。 プロジェクトには、COM コンポーネント、XML Web サービス、.NET Framework クラス ライブラリまたはアセンブリ、その他のクラス ライブラリの参照を含めることができます。 参照の使用方法については、「[プロジェクト内の参照の管理](../../ide/managing-references-in-a-project.md)」を参照してください。
 ...
 **追加** 参照または Web 参照を **参照リストに** 追加するには、このボタンをクリックします。
 ...
 [Web 参照の追加] ダイアログボックスを使用して、プロジェクトに Web 参照を追加するには、[ **Web 参照** ] を選択します。
 ...
 **Web 参照の更新** [ **参照** ] の一覧で Web 参照を選択し、このボタンをクリックして更新します。
 ...
 ユーザーがインポートした名前空間にエイリアスを作成できます。 これを行うには *、alias* = *名前空間*にエイリアスと名前空間を入力します。 これは、`Http= MyOrg.ObjectLib.Internet.WebRequestMethods.Http` のような、長い名前空間を使用する場合に便利です。
 ...
 **ユーザーインポートの更新** [名前空間] ボックスの一覧でユーザー指定の名前空間を選択し、[ **インポート** された名前空間] ボックスで置き換える名前を入力します。次に、このボタンをクリックして、新しい名前空間に移動します。 このボタンは、選択した名前空間が **[ユーザー インポートの追加]** ボタンを利用して一覧に追加した名前空間の場合にのみ表示されます。 以下を追加できます。
