...
- 日付の書式を正す。
  ...
  **インターフェイスと MustOverride メンバーの自動挿入**`Implements`クラスのステートメントまたはステートメントをコミットすると、 `Inherits` 実装またはオーバーライドする必要があるメンバーのプロトタイプがテキストエディターによってそれぞれ挿入されます。
 ...
 **参照とキーワードの強調表示を有効にする** テキストエディターでは、、、またはなどの句で、シンボルのすべてのインスタンス、またはすべてのキーワードを強調表示でき `If..Then` `While...End While` `Try...Catch...Finally` ます。 強調表示された参照間またはキーワード間を移動するには、Ctrl キーと Shift キーを押しながら下方向キーを押すか、Ctrl キーと Shift キーを押しながら上方向キーを押します。
