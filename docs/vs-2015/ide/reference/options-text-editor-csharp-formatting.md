...
## <a name="remarks"></a>注釈
 各言語の [ **タブ** ] ページのインデントオプションでは、行の最後で enter キーを押したときに、コードエディターがカーソルを配置する場所が決まります。 **[書式設定]** のインデント オプションは、たとえば、**[貼り付け時にインデントを調整する]** チェック ボックスがオンになっている間にコードをファイルに貼り付けるときなど、コードが自動的に書式設定される場合に適用されます。また、フォーマットされるブロックを手動で入力する場合にも適用されます。
