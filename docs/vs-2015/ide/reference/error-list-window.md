...
注意]
> エラー一覧には、特定のエラー メッセージに関する情報が表示されます。 [出力] ウィンドウから、エラー番号またはエラー文字列のテキストをコピーできます。 [出力] ウィンドウを表示するには、Ctrl + Alt + O キーを押します。 「 [出力ウィンドウ](../../ide/reference/output-window.md)」を参照してください。
