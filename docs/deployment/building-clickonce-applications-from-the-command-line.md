...
## <a name="publish-properties"></a>[発行] プロパティ
 上記の手順でアプリケーションを発行すると、発行ウィザードによって次のプロパティがプロジェクトファイルに挿入されます。 これらのプロパティは、アプリケーションの生成方法に直接影響 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] します。
...
```
 ...
 これらのプロパティは、プロジェクトファイル自体を変更せずに、コマンドラインでオーバーライドできます。 たとえば、次のようにすると、ブートストラップを使用せずにアプリケーションの展開がビルドされ [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] ます。
