...
|------------------|------------------|
|Visio|\\*Visio*\\*アドイン ID*の*ルート*の \Software\Microsoft|
|その他すべて|*ルート*\Software\Microsoft\Office\\*Office アプリケーション名*\ アドイン\\*アドイン ID*|

> [!NOTE]
> インストーラーが64ビットの Windows 上のすべてのユーザーを対象としている場合は、2つのレジストリエントリ (HKEY_LOCAL_MACHINE \Software\Microsoft の下に1つ、 **WOW6432Node**\Microsoft hive に\\HKEY_LOCAL_MACHINE 1 つずつ) を含めることをお勧めします。 これは、ユーザーがコンピューターで32ビット版または64ビット版の Office を使用できる可能性があるためです。
>
>インストーラーが現在のユーザーを対象としている場合は、HKEY_CURRENT_USER \ ソフトウェアのパスが共有されているため、WOW6432Node にインストールする必要はありません。
>
