...
|------------------|------------------|
|Visio|*ルート*\Software\Microsoft \\ *Visio* \\ *の Visio アドイン ID*|
|その他すべて|*ルート*\Software\Microsoft\Office \\ *Office アプリケーション名*\ アドイン \\ *ID*|

> [!NOTE]
> インストーラーが64ビットの Windows 上のすべてのユーザーを対象としている場合は、2つのレジストリエントリ (HKEY_LOCAL_MACHINE \Software\Microsoft の下と HKEY_LOCAL_MACHINE \ software WOW6432Node \Microsoft hive の両方) を含めることをお勧めし \\ **WOW6432Node**ます。 これは、ユーザーがコンピューターで32ビット版または64ビット版の Office を使用できる可能性があるためです。
>
...
|**フレンドリ**|REG_SZ|必須。 Microsoft Office アプリケーションの **[COM アドイン]** ダイアログ ボックスに表示される、VSTO アドインの説明的な名前。 既定値は VSTO アドイン ID です。|
 ...
 *ルート*\Software\Microsoft\Office アプリケーション名**LOADBEHAVIOR**のアドイン ID キーの下にあるエントリには、 \\ *application name* \\ *add-in ID* VSTO アドインの実行時の動作を指定する値のビットごとの組み合わせが含まれています。 最下位のビット (値 0 および 1) は、VSTO アドインが現在アンロードされているか、または読み込み済みであるかを示します。 その他のビットは、アプリケーションが VSTO アドインを読み込もうとしていることを示します。
