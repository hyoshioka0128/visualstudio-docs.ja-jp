...
description: ワークフローデザイナーで ReceiveAndSendReply テンプレートを使用して、事前に構成された Receive アクティビティと SendReply アクティビティのペアを作成する方法について説明します。
...
| プロパティ名 | 必須 | 使用 |
|-|----------|-|
| <xref:System.Activities.Activity.DisplayName%2A> | False | <xref:System.ServiceModel.Activities.SendReply> アクティビティの省略可能な表示名。 既定値は SendReplyToReceive です。<br /><br /> フレンドリに既定値以外の値を使用 <xref:System.Activities.Activity.DisplayName%2A> することは、厳密には必須ではありませんが、このような値を使用することをお勧めします。 |
