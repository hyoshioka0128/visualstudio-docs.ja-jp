...
1. Visual Studio に切り替えて、JavaScript ファイル、TypeScript ファイル、JSX ファイルなどのソース コードにブレークポイントを設定します。 (return ステートメントや var 宣言など、ブレークポイントが許可されるコード行にブレークポイントを設定します)。
    ...
    クライアント側コードの場合、TypeScript ファイル、または JSX ファイル内のブレークポイントをヒットさせるには、通常、[ソース マップ](#generate_source_maps)を使用する必要があります。 ソース マップは、Visual Studio でのデバッグをサポートするように正しく構成されている必要があります。
