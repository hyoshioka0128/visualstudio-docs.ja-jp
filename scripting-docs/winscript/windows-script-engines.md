...
|-|-|  
|カテゴリ|説明|  
|CATID_ActiveScript|クラス識別子 (Clsid) が、少なくとも[IActiveScript](../winscript/reference/iactivescript.md)インターフェイスと永続化機構 (、、 `IPersistStorage` `IPersistStreamInit` または IPersistPropertyBag インターフェイス) をサポートする Windows スクリプトエンジンであることを示します。|  
...
|closed|スクリプトは終了しています。 スクリプト エンジンは動作しなくなり、ほとんどのメソッドに対してエラーを返します。|  
